...
periods, underscores, percent signs, plus signs, or hyphens.
...
describe('PeEmailValidator', () => {
  const cases: [string[], ValidationErrors | null][] = [
    [validEmails, null],
  ...
  cases.forEach(([emails, expected]) => {
    ...
    test.each(emails)(`email: %s should mark as ${validity}`, (email) => {
      ...
      expect(validator.emailValidator(control)).toEqual(expected);
    });
  });

  it('should handle required validate', () => {

    const emailValidator = jest.spyOn(validator, 'emailValidator');

    expect(validator.emailRequiredValidator(new FormControl(''))).toEqual({ required: true });
    expect(emailValidator).not.toHaveBeenCalled();
    validEmails.forEach((email) => {
      const formControl = new FormControl(email);
      expect(validator.emailValidator(formControl)).not.toEqual({ required: true });
      expect(emailValidator).toHaveBeenCalledWith(formControl);
    });
