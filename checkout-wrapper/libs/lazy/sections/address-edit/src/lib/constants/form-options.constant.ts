import { SelectOptionInterface } from '@pe/checkout/types';

export const SALUTATION_OPTIONS: SelectOptionInterface[] = [
  {
    label: $localize`:@@checkout_address_edit.form.salutation.value.SALUTATION_MR:`,
    value: 'SALUTATION_MR',
  },
  {
    label: $localize`:@@checkout_address_edit.form.salutation.value.SALUTATION_MRS:`,
    value: 'SALUTATION_MRS',
  },
];

export const SELECT_ADDRESS_OPTIONS: SelectOptionInterface[] = [
  {
    label: $localize`:@@checkout_address_edit.form.new_addresses.label:`,
    value: null,
  },
];

