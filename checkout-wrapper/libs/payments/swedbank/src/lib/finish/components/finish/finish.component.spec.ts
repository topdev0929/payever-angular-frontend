...
describe('FinishComponent', () => {
  ...
  it('should isNewStatus return correct value', () => {

    component.payment = { status: PaymentStatusEnum.STATUS_IN_PROCESS };

    fixture.detectChanges();

    expect(component.isNewStatus()).toBeFalsy();

    component.payment = { status: PaymentStatusEnum.STATUS_NEW };

    fixture.detectChanges();

    expect(component.isNewStatus()).toBeTruthy();

  });

  it('should getNodeResultDetails successfully return paymentDetails', () => {

    component.nodeResult = { paymentDetails: 'test' } as any;

    expect(component.getNodeResultDetails()).toEqual( 'test');
