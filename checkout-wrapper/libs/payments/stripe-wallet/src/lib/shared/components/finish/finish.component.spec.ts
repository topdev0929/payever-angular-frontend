...
describe.only('stripe-wallet-shared-finish', () => {
  let store: Store;
  ...
  describe('component', () => {
    ...
    it('should get correct translations if isShowingWallet true ', () => {
      component.isShowingWallet = true;
      expect(component.translations).toMatchObject({
        processingTitle: $localize `:@@payment-stripe-wallet.inquiry.finish.wallet.title:`,
        processingText: $localize `:@@payment-stripe-wallet.inquiry.finish.wallet.text:`,
      });
    });
    it('should get correct translations if isShowingWallet false ', () => {
      component.isShowingWallet = false;
      expect(component.translations).toMatchObject({
        processingTitle: $localize `:@@payment-stripe-wallet.inquiry.finish.processing.title:`,
        processingText: $localize `:@@payment-stripe-wallet.inquiry.finish.processing.text:`,
      });
    });
    ...
    describe.only('isStatusPending', () => {
      const shouldReturnTrue = [
        PaymentStatusEnum.STATUS_IN_PROCESS,
      ];
      expectStatus(shouldReturnTrue, () => component.isStatusPending());
    });
    describe.only('isStatusFail', () => {
      const shouldReturnTrue = [
        PaymentStatusEnum.STATUS_FAILED,
