...
import { InquireFormIdentifyBorrowerComponent } from './inquire-form-identify-borrower.component';

describe('InquireFormIdentifyBorrowerComponent', () => {
  const storeHelper = new StoreHelper();

  let component: InquireFormIdentifyBorrowerComponent;
  let fixture: ComponentFixture<InquireFormIdentifyBorrowerComponent>;
  let store: Store;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(InquireFormIdentifyBorrowerComponent);
  ...
  describe('Form Submission', () => {
    ...
    it('should dispatch PatchFormState and emit submitted event on onSubmit if form is valid', () => {
      const mockValidValue: IdentifyFormValue = {
      ...
      const mockFormValue = { borrower: { _identifyForm: {} } };
      component.formGroup.patchValue({ _identifyForm: mockValidValue });
      jest.spyOn(component.formGroup, 'valid', 'get').mockReturnValue(true);
      jest.spyOn(store, 'selectSnapshot').mockReturnValue(mockFormValue);
      jest.spyOn(store, 'dispatch');
      jest.spyOn(component.submitted, 'emit');

      component.onSubmit();

      expect(component.submitted.emit).toHaveBeenCalledWith(mockValidValue);
    });
