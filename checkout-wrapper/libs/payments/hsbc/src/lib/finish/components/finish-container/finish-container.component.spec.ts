...
import { PaymentService } from '../../../shared/service';
...
describe('FinishContainerComponent', () => {
  ...
  it('should show finish modal from existing payment', () => {
    ...
    const getFinalResponseSpy = jest
      .spyOn(component['nodeFlowService'], 'getFinalResponse')
      .mockReturnValue({
        payment: {},
      } as PaymentResponse);
  ...
  it('should handle error when getting payment', () => {
    ...
    const updatePayment = jest.spyOn(component['nodeFlowService'], 'updatePayment')
