...
export class PaymentService extends AbstractPaymentService {
  ...
  private preparePayment(shopUrls: NodeShopUrlsInterface): Observable<void> {
    ...
    nodePaymentDetails.frontendCancelUrl = shopUrls.cancelUrl || `${checkoutWrapper}/${lang}/pay/${this.flow.id}/redirect-to-choose-payment`;
    if (this.flow.apiCall?.birthDate) {
      nodePaymentDetails.birthdate = this.dateUtilService.fixDate(this.flow.apiCall.birthDate);
    }
