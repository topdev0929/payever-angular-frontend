...
describe.only('stripe-direct-debit-shared-finish', () => {
  let store: Store;
  ...
  describe('component', () => {
    describe('checkout-sdk-finish-status-success', () => {
      ...
      it('Should have mandateUrl', () => {
        const { childEl } = QueryChildByDirective(fixture, FinishStatusSuccessComponent);
        const mandateUrl = childEl.nativeElement.querySelector('#mandateUrl');
        expect(mandateUrl).toBeTruthy();
        const aHref = mandateUrl.querySelector('a');
        expect(aHref).toBeTruthy();
        expect(aHref.getAttribute('href')).toEqual('mandate-url');
      });
