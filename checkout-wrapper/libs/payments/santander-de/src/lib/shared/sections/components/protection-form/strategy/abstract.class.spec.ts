...
import { PatchPaymentDetails, PaymentState, SetFlow, SetPayments } from '@pe/checkout/store';
...
describe('AbstractInsuranceStrategyClass', () => {
  ...
  beforeEach(() => {
    ...
    store.dispatch(new PatchPaymentDetails({ rate: mockRatesData.rates[1] }));
  ...
  it('should get credit duration in months from state', () => {
    expect(classInstance.creditDurationInMonths).toEqual(mockRatesData.rates[1].duration);
  });

  it('should get rate', () => {
    expect(classInstance.rate).toMatchObject(mockRatesData.rates[1]);
  });
  ...
  it('should get cpiRate', () => {
    expect(classInstance.cpiRate).toEqual(mockRatesData.rates[1]);
  });
  ...
  it('should correct create month format', () => {
    ...
    expect(classInstance['toMonthFormat'](1)).toEqual(`1 ${$localize`:@@payment-santander-de-pos.creditRates.month:`}`);
    expect(classInstance['toMonthFormat'](10)).toEqual(`10 ${$localize`:@@payment-santander-de-pos.creditRates.months:`}`);
