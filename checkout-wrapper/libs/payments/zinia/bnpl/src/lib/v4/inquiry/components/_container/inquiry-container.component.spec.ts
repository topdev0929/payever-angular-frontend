...
describe('zinia-bnpl-inquiry-container-v4', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        importProvidersFrom(ZiniaBNPLInquiryModuleV4),
  ...
  describe('Constructor', () => {
    it('Should create an instance', () => {
      ...
      const initPaymentMethod = jest.spyOn(TestBed.inject(AnalyticsFormService), 'initPaymentMethod');
      const nodeInitFor = jest.spyOn(TestBed.inject(ThreatMetrixService), 'nodeInitFor');
      component.ngOnInit();
      const flow = store.selectSnapshot(FlowState.flow);
      const paymentMethod = store.selectSnapshot(FlowState.paymentMethod);
      expect(initPaymentMethod).toHaveBeenCalledWith(paymentMethod);
      expect(nodeInitFor).toHaveBeenCalledWith(
        flow.id,
        flow.connectionId,
        paymentMethod,
      );
