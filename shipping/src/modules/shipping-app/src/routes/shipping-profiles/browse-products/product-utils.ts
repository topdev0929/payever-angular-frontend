...
import { PeChannelGroup, PE_CHANNELS_GROUPS } from '../../../interfaces/products/channel-group.interface';
...
export function getProductCategory(product: Product): string {
  const categories: string[] = (product.categories || []).map((category) => category && category.title).filter((title) => title);
...
export function getTrackableProductInventory(product: Product, inventories: InventoryInterface[]): InventoryInterface[] {
  const skus: string[] = product.variants && product.variants.length ? product.variants.map((variant) => variant.sku) : [product.sku];
