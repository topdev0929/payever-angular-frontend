...
@Component({
  selector: 'peb-soft-package',
  templateUrl: './soft-package.component.html',
  styleUrls: ['./soft-package.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class PebSoftPackageComponent extends PebPackageBaseComponent implements OnInit, OnDestroy {
  ...
  getItems() {
    this.packageService.getSoftPackage().subscribe((softPackages: ShippingBoxInterface[]) => {
      this.items = [];
      softPackages.forEach((softPackage) => {
        this.items.push({
          id: softPackage._id,
          labels: softPackage.isDefault ? ['Default'] : [],
          ...
          ">
            ...
            ">
              <span style="
                font-weight: bold;">
                ${softPackage.type.substr(0, 4)}
              </span>
            </div>
            <span>${softPackage.name}</span>
          </div>
          `),
          customFields: [
            {
              content: this.sanitizer.bypassSecurityTrustHtml(`
                <span>${softPackage.width} ${softPackage.dimensionUnit}</span>
            `),
            },
            {
              content: this.sanitizer.bypassSecurityTrustHtml(`
                <span>${softPackage.height} ${softPackage.dimensionUnit}</span>
            `),
            },
            {
              content: this.sanitizer.bypassSecurityTrustHtml(`
                <span>${softPackage.length} ${softPackage.dimensionUnit}</span>
            `),
            },
            {
              content: this.sanitizer.bypassSecurityTrustHtml(`
                <span>${softPackage.weight} ${softPackage.weightUnit}</span>
            `),
            },
            {
              content: this.sanitizer.bypassSecurityTrustHtml(`
