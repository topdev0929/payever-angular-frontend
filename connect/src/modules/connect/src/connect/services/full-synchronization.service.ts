...
export class FullSynchronizationService {
  ...
  getValue(businessId: string, integrationId: string): Observable<boolean> {
    return this.http.get<boolean>(`${this.config().synchronizer}/api/synchronization/business/${businessId}/integration/${integrationId}/settings/status`);
  }

  saveValue(businessId: string, integrationId: string, enable: boolean): Observable<void> {
    return this.http.patch<void>(`${this.config().synchronizer}/api/synchronization/business/${businessId}/integration/${integrationId}/settings/${enable ? 'enable' : 'disable'}`, {});
  }
