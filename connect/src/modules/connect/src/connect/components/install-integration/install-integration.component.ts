...
export class InstallIntegrationComponent implements OnInit, OnDestroy, AfterViewInit { // TODO Refactor. Just show 'integration-card'
  ...
  toggleInstalled(): void {
    ...
    if (!this.isInstallingSubject.getValue()) {
      ...
      this.integrationsStateService.installIntegration(this.name, this.paymentsStateService, !this.installed).subscribe(integration => {
        this.isDone = true;
