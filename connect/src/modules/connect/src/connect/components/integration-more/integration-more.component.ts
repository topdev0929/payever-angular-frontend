...
export class IntegrationMoreComponent extends AbstractComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (get(changes, 'integration.currentValue.category')) {
      this.integrationsStateService.getCategoryIntegrations(false, [this.integration.category], true).pipe(
  ...
  navigate() {
    ...
    this.router.navigate(
      [`business/${businessId}/connect/${this.integration.category}/`], { queryParams: {integrationName: this.integration.name} }
    );
  }
}
