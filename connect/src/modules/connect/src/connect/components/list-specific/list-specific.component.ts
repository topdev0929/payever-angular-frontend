...
import { flatMap, takeUntil, filter, take } from 'rxjs/operators';
...
*/
  ...
  ngOnInit(): void {
    ...
    const list$ = this.integrationsStateService.getCategoryIntegrations(false, [this.specificCatrgory] as IntegrationCategory[]).pipe(
  ...
  saveReturn(category: IntegrationCategory): void {
    ...
    if (this.microPath) {
      this.navigationService.saveReturn(`business/${businessId}/connect/back-to-micro/${this.micro}/micro-path/${this.microPath}`);
    } else if (this.entityUuid) {
      this.navigationService.saveReturn(`business/${businessId}/connect/back-to-micro/${this.micro}/category/${category}/${this.entityName}/${this.entityUuid}`);
    } else {
      this.navigationService.saveReturn(`business/${businessId}/connect/back-to-micro/${this.micro}/category/${category}`);
    }
  }
  ...
  installIntegrationDirectly(integration: IntegrationInfoWithStatusInterface): void {
    if (!this.installingIntegration) {
      ...
      this.integrationsStateService.installIntegration(integration.name, this.paymentsStateService, true).subscribe(() => {
        const businessId = this.integrationsStateService.getBusinessId();
        if (this.entityUuid) {
          this.navigationService.saveReturn(`business/${businessId}/connect/back-to-micro/${this.micro}/category/${integration.category}/${this.entityName}/${this.entityUuid}`);
        } else {
          this.navigationService.saveReturn(`business/${businessId}/connect/back-to-micro/${this.micro}/category/${integration.category}`);
        }
