...
export class IntegrationRatingStarsComponent extends AbstractComponent implements OnChanges {
  ...
  EMPTY_STAR: string = '#icon-empty-star';
  FILLED_STAR: string = '#icon-filled-star';

  @Input() decimalMode: boolean = false;
  @Input() currentRating: number = 0;
  @Input() maxRating: number = 5;
  @Input() readonly: boolean = false;
  @Input() iconClass: string = 'icon-16';
  ...
  @ViewChild('stars', { read: ElementRef }) set stars(stars: ElementRef) {
    setTimeout(() => {
      ...
      if (stars) {
        this.filledInStarsContainerWidth = this.currentRating / this.maxRating * this.starsElement.nativeElement.clientWidth;
      }
