...
const APP_DETAILS_TEXT = 'list_common.title';
const APP_DETAILS_ICON = 'icon-apps-app-market';
/*
...
/*    const config: PePlatformHeaderConfig = {
  ...
  initCheckoutWelcomeHeader(title: string, snapshot: ActivatedRouteSnapshot) {
    ...
    const checkoutWelcomeScreen = queryParams['checkoutWelcomeScreen'] === true || queryParams['checkoutWelcomeScreen'] === 'true';
    if (checkoutWelcomeScreen && checkoutUuid) {
      ...
      this.hideHeader();
      this.simpleStepperService.show(this.translateService.hasTranslation(title) ? this.translateService.translate(title) : title, [
        {
