import { Injector, OnInit, OnDestroy, Directive, ChangeDetectorRef } from '@angular/core';
...
export abstract class BaseListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // We pass true to force call GET /.../variants again on each app open. It was requested by backend.
    this.paymentsStateService.getPaymentsWithVariant(true).pipe(filter(d => !!d), take(1)).subscribe(() => {
