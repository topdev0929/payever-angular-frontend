...
export class ContactHelperService {
  ...
  salutations: SelectOptionInterface[] = [
    ...
    },
    {
      ...
      label: this.translateService.translate('user_business_form.form.contactDetails.salutation.options.SALUTATION_MRS')
    }
  ];

  constructor(addressService: AddressService, private translateService: TranslateService, private formBuilder: FormBuilder) {
  }

  createForm(initialData: ContactFormFieldsInterface, requiredFields: string[]): { form: FormGroup, formSchemeFields: FormSchemeField[] } {
    ...
    const form: FormGroup = this.formBuilder.group({
      ...
      additionalPhone: [initialData.additionalPhone || '', requiredFields.indexOf('contactDetails.additionalPhone') >= 0 ? req : []]
    });
