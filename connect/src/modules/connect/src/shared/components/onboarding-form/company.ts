...
import { EnvironmentConfigInterface as EnvInterface } from '@pe/common/environment-config/interfaces/environment-config.interface';
...
export class CompanyHelperService {
  ...
  createForm(initialData: CompanyFormFieldsInterface, requiredFields: string[]): { form: FormGroup, formSchemeFields: FormSchemeField[] } {
    const req = [ Validators.required ];
    this.form = this.formBuilder.group({
      // TODO Must be also added 'employeesRange' and 'salesRange'
      legalForm:    [initialData.legalForm    || '', requiredFields.indexOf('companyDetails.legalForm')    >= 0 ? req : []],
      product:      [initialData.product      || '', requiredFields.indexOf('companyDetails.product')      >= 0 ? req : []],
      industry:     [initialData.industry     || '', requiredFields.indexOf('companyDetails.industry')     >= 0 ? req : []],
      urlWebsite:   [initialData.urlWebsite   || '', requiredFields.indexOf('companyDetails.urlWebsite')   >= 0 ? req : []],
      foundationYear: [initialData.foundationYear || '',
        requiredFields.indexOf('companyDetails.foundationYear') >= 0 ? [ ...req, this.getYearValidator() ] : [ this.getYearValidator() ]
      ]
    });
    ...
    const formSchemeFields: FormSchemeField[] = [
      ...
      },
      {
        ...
        fieldSettings: {
          ...
          required: requiredFields.indexOf('companyDetails.product') >= 0 || requiredFields.indexOf('companyDetails.industry') >= 0
        },
  ...
  private updateIndustryValues(product: string): void {
    if (product && this.productWithIndustries) {
      this.industryValues$.next(this.productWithIndustries.find(prod => prod.code === product).industries.map(industry => {
        return {
          value: industry.code,
