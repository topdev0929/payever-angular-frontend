...
export class TaxesHelperService {
  ...
  createForm(initialData: TaxesFormFieldsInterface, requiredFields: string[]): { form: FormGroup, formSchemeFields: FormSchemeField[] } {
    const req = [ Validators.required ];
    const form: FormGroup = this.formBuilder.group({
      companyRegisterNumber: [initialData.companyRegisterNumber || '', requiredFields.indexOf('taxes.companyRegisterNumber') >= 0 ? req : []],
      ...
      turnoverTaxAct: [initialData.turnoverTaxAct || false, requiredFields.indexOf('taxes.turnoverTaxAct') >= 0 ? req : []]
    });
