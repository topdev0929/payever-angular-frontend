...
export class BankHelperService {
  ...
  createForm(initialData: BankFormFieldsInterface, requiredFields: string[]): { form: FormGroup, formSchemeFields: FormSchemeField[] } {
    ...
    const form: FormGroup = this.formBuilder.group({
      ...
      routingNumber: [initialData.routingNumber || '', requiredFields.indexOf('bankAccount.routingNumber') >= 0 ? req : []],
      accountNumber: [initialData.accountNumber || '', requiredFields.indexOf('bankAccount.accountNumber') >= 0 ? req : []]
    });
  ...
  onUpdateFormData(toggleControl: (name: string, enable: boolean) => void, formsValues: BankFormInterface) {
    if (formsValues.bankAccount) {
      // console.log(formsValues.bankAccount);
