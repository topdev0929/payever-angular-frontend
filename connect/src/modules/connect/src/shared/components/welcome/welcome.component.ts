...
export class ConnectWelcomeComponent implements OnInit {
  ...
  getStarted() {
    this.isLoading = true;
    this.integrationsApiService.setStatus(this.businessService.getBusinessId())
      .pipe(
        switchMap(() => this.integrationsApiService.startTrial(this.businessService.getBusinessId())),
        finalize(() => {
          this.isLoading = false;
          this.onAction.next(1);
          this.router.navigate([`business/${this.businessService.getBusinessId()}/connect`]);
        })
      )
      .subscribe(() => {
      });
  ...
  backToDashboard() {
    this.onAction.next(1);
    this.router.navigate([`business/${this.businessService.getBusinessId()}/info/overview`]);
  }
}
