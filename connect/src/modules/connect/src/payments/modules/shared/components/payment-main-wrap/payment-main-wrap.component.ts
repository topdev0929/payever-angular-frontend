...
import { IntegrationsStateService, PanelEnum, PaymentMethodEnum, PaymentWithVariantInterface, StepEnum, UninstallService, VariantListItemInterface } from '../../../../../shared';
...
export class PaymentMainWrapComponent extends BasePaymentAccordionComponent implements OnInit, AfterViewChecked {
  ...
  constructor(injector: Injector) {
    ...
    // For panels:
    this.paymentData$ = this.payment$.pipe(takeUntil(this.destroyed$), filter(d => !!d), distinctUntilChanged((prev, next) => {
  ...
  showConnectionVariantModal(): void {
    if (this.isVariantStatusConnected(0)) {
      ...
      this.showStepError(this.translateService.translate('categories.payments.add_variant.errors.connect_default_before_add'));
  ...
  disconnectConnectionVariant(event: Event, index: number): boolean {
    ...
    if (this.processingConnVariant$.value < 0) {
      ...
      this.paymentsStateService.resetCredentails(this.payment, this.payment.variants[index]).pipe(takeUntil(this.destroyed$)).subscribe(data => {
        // this.onClearCredentials(); // TODO
        this.changeDetectorRef.detectChanges();
        this.processingConnVariant$.next(-1);
        this.restoreLastExpandedPanel();
      }, error => {
  ...
  deleteConnectionVariant(event: Event, index: number): boolean {
    ...
    if (this.processingConnVariant$.value < 0) {
      this.processingConnVariant$.next(index);
      this.paymentsStateService.deleteConnectionVariant(this.payment, this.payment.variants[index]).pipe(takeUntil(this.destroyed$)).subscribe(data => {
        // this.onClearCredentials(); // TODO
        this.changeDetectorRef.detectChanges();
        this.processingConnVariant$.next(-1);
      }, error => {
