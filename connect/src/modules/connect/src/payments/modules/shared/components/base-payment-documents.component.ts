...
export abstract class BasePaymentDocumentsComponent extends BasePaymentComponent {
  ...
  constructor(
    ...
    super(injector);
    this.payment$.pipe(takeUntil(this.destroyed$), filter(d => !!d)).subscribe((payment: PaymentWithVariantInterface) => {
      if (!this.steps) {
        ...
        payment.missing_steps.missing_steps.filter(step => STEP_UPLOAD_TYPES.indexOf(step.type) >= 0).forEach(stepData => {
      ...
      this.paymentPayload$ = this.paymentsStateService.getConnectPaymentPayload(this.paymentMethod).pipe(filter(d => !!d));
    });
  }
  ...
  getStepDownload(step: StepInterface): string {
    ...
    if (step.url) {
      ...
      result = `${config.payments}/api/rest/v1/business-payment-option/${this.paymentsStateService.getBusinessUuid()}/get-document/${step.url.split('/').pop()}`;
    } else if (this.hardcodedDocuments[step.type]) {
  ...
  downloadUploaded(step: StepInterface): void {
    this.paymentPayload$.pipe(take(1)).subscribe(paymentPayload => {
      ...
      const url: string = this.mediaUrlPipe.transform(paymentPayload.documents.find(a => a.type === doc).blobName, 'images');
      if (url) {
  ...
  runStepUpload(step: StepInterface, event: Event): void {
    ...
    if (!this.isFileSizeValid(file)) {
      this.snackBarService.show(this.translateService.translate('ng_kit.forms.error.image_picker.max_size', { size: MAX_FILE_SIZE_TEXT }));
      return;
    }

    this.getStepUploadProgress(step).next(0);
    this.mediaService.createBlobByBusiness(this.paymentsStateService.getBusinessUuid(), MediaContainerType.Images, file).subscribe(
