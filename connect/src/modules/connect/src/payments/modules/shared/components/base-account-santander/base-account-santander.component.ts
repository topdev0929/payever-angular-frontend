import { EventEmitter, Injector, Output, QueryList, Directive, ChangeDetectorRef } from '@angular/core';
/*
...
import { IntegrationsStateService, PaymentMethodEnum, UserBusinessInterface } from '../../../../../shared';
...
// export abstract class BaseAccountSantanderComponent extends BasePaymentFormComponent<FormInterface> {
  ...
  onSubmitSuccess(data: UserBusinessInterface): void {
    this.isLoading = true;
    this.paymentsStateService.refreshUserBusinesses(this.paymentMethod).pipe(
      flatMap(() => {
        return this.paymentsStateService.saveConnectPaymentPayload(this.paymentMethod, { application_sent: true }).pipe(
          map(() => null)
        );
      })
    ).subscribe(() => {
