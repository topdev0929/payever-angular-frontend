...
export class StepsHelperService {
  ...
  hasSectionAccountBody(payment: PaymentWithVariantInterface): boolean {
    ...
    return this.getStep(payment, StepEnum.additionalInfo) && (
      ...
      !this.getStep(payment, StepEnum.additionalInfo).filled || payment.variants.filter(variant => variant.status === 'new').length > 0
    );
  }
  ...
  hasSectionDocuments(payment: PaymentWithVariantInterface): boolean {
    return !!STEP_UPLOAD_TYPES.find(uploadType => !!this.getStep(payment, uploadType)) && !this.isAllStatusConnected(payment);
  }
  ...
  isSectionDocumentsFilled(payment: PaymentWithVariantInterface): boolean {
    ...
    return !STEP_UPLOAD_TYPES.find(uploadType => this.getStep(payment, uploadType) && !this.getStep(payment, uploadType).filled);
  }
  ...
  */
  ...
  isVariantStatusConnected(payment: PaymentWithVariantInterface, paymentIndex: number): boolean {
    return payment.variants[paymentIndex] && (payment.variants[paymentIndex].credentials_valid || this.isFilledExternalAuthentication(payment));
  }
  ...
  showStepError(error: string): void {
    ...
    snackBarService.toggle(true, error || this.translateService.translate('errors.unknown_error'), {
      duration: 5000,
  ...
  private isFilledExternalAuthentication(payment: PaymentWithVariantInterface): boolean {
    return !!this.getStep(payment, StepEnum.missingExternalAuthentication) && this.getStep(payment, StepEnum.missingExternalAuthentication).filled;
  }
}
