...
export abstract class BaseAuthVendorIdsComponent extends BaseAuthComponent<FormInterface> {
  ...
  formScheme: FormScheme = {
    fieldsets: {
      credentials: [
        this.prepareFieldset('vendorId0', null, { classList: 'col-xs-12 form-fieldset-field-padding-24 no-border-radius' }),
  ...
  private prepareFieldset(name: string, addonAppend: AddonInterface, fieldSettings: FieldSettingsInterface = null): FormSchemeField {
  ...
  onSuccess() {
    ...
    this.paymentsStateService.saveCredentials({ credentials: { vendorIds: this.getFinalValues() } }, this.payment, this.payment.variants[this.paymentIndex])
