...
export abstract class BaseAuthStoreIdMerchantNumberStoreIdentifier extends BaseAuthComponent<FormInterface> {
  ...
  createFormDeferred(initialData: FormInterface) {
    ...
    this.form = this.formBuilder.group({
      credentials: this.formBuilder.group({
        ...
        useDefaultStoreIdentifier: [credentials['useDefaultStoreIdentifier'] || initialData.credentials.useDefaultStoreIdentifier || false],
        storeIdentifier: [credentials['storeIdentifier'] || initialData.credentials.storeIdentifier, Validators.required],
      })
    });
  ...
  protected onUpdateFormData(formValues: FormInterface): void {
    this.toggleControl('credentials.storeIdentifier', formValues.credentials && !formValues.credentials.useDefaultStoreIdentifier);
  }
}
