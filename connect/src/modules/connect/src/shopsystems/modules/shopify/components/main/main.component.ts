...
import {
  ApiKeysBaseComponent, AuthTokenInterface, BusinessService, IntegrationCategory, IntegrationInfoWithStatusInterface, IntegrationsStateService, NavigationService, UninstallService
} from '../../../../../shared';
...
export class ShopifyMainComponent extends ApiKeysBaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.integrationInfo$ = this.integrationsStateService.getIntegration(this.integrationName).pipe(takeUntil(this.destroyed$));
    this.integrationInfo$.pipe(finalize(() => this.onDataLoad.next(1))).subscribe(info => {
      this.integration = info;
    });

    this.paymentIntegrations$ = this.integrationsStateService.getCategoryIntegrations(true, [IntegrationCategory.Payments]).pipe(
      filter(a => !!a),
