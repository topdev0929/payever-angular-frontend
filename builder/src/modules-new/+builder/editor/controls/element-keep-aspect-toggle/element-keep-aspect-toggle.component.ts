...
@Component({
  selector: 'pe-builder-element-keep-aspect-toggle',
  templateUrl: './element-keep-aspect-toggle.component.html',
  styleUrls: ['./element-keep-aspect-toggle.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ElementKeepAspectToggleComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.checked$ = combineLatest([this.editor.activeElement$, this.pageStore.state$]).pipe(
      ...
      map((element: PebElement) => element.meta.keepAspect),
    );
  }

  onChanged(keepAspect: boolean): void {
