import { ChangeDetectorRef, Component, ElementRef, HostBinding, Inject, Input, ViewChild, } from '@angular/core';
...
import { ExecuteCommandAction, TextEditorComponent, TextEditorService, TextEditorToolbarComponent } from '../texteditor';

const TOOLBAR_PADDING: number = 15;
const EDITOR_SIZE = {
...
export class PeInvoiceSettingsEmailComponent {
  ...
  @HostBinding('id') hostId: string = 'editor-description';
  ...
  validateForms() {
    for (let control in this.emailForm.controls) {
      ...
      if (this.emailForm.controls[control].invalid) {
        ...
        if(['inbox_userName', 'inbox_password', 'inbox_server', 'inbox_port', 'inbox_protection'].includes(control)) {
