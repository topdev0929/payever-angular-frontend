...
import { PEB_PRODUCTS_API_PATH, PEB_STORAGE_PATH } from '@pe/builder-api';
import { PebProduct } from '@pe/builder-core';
...
export class CommonService {
  ...
  getContacts(filterStr: string = '', limit = 20, offset = 1): Observable<any> {
    ...
    return this.http.get(`${this.contactsApiPath}/api/es/list/${this.envService.businessId}?filters=${filters}&page=${offset}&limit=${limit}`).pipe(
      filter((result: any) => !!result?.collection),
      map((result: any) => {
    ...
    //   }}`,
    // }).pipe(
    //   filter((result: any) => !!result?.data?.contacts),
    //   map((result: any) => {
    //     let contacts = result.data.contacts.map(contact => ({
    //         id: contact._id,
    //         business: contact.businessId,
    //         name: `${contact.fields.find(item => item.field.name === "firstName")?.value}
    //         ${contact.fields.find(item => item.field.name === "lastName")?.value}`,
    //         image: contact.fields.find(item => item.field.name === "imageUrl")?.value,
    //         email: contact.fields.find(item => item.field.name === "email")?.value
    //       })
    //     );
    //     return contacts;
    //   })
