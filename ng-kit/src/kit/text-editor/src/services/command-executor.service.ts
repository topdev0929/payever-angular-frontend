...
import { EMPTY_CHAR, ESCAPE_CHAR, ExecuteCommands, TextOptionsInterface } from '../common';
import * as Utils from '../common/text-editor.utils';
import { DataGridDefaultFilterConditions } from '../../../data-grid/src/const';
import number = DataGridDefaultFilterConditions.number;
...
export class CommandExecutorService {
  ...
  setListColor(range: Range, color: string, renderer: Renderer2): void {
    if (this.selection && range && Utils.hasList()) {
      ...
      if (!range.collapsed && Utils.isHtmlElement(range.commonAncestorContainer)
        && (range.commonAncestorContainer as HTMLElement).tagName.toUpperCase() === 'UL') {
        this.execute(ExecuteCommands.TEXT_COLOR, color);
        for (let i: number = 0; i < range.commonAncestorContainer.childNodes.length; i++) {
  ...
  private cleanNodeChildsFontSize(node: DocumentFragment | HTMLElement, renderer: Renderer2): void {
    ...
    for (let i: number = 0; i <  styledElements.length; i++) {
