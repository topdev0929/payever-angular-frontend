...
import { DynamicInfoBoxGeneratorFormData, InfoBoxGeneratorFormComponent } from '../info-box-generator-form/info-box-generator-form.component';
...
export class InfoBoxGeneratorComponent implements OnInit {
  ...
  isShowDebugButtonText: boolean = false;
  ...
  private get firstSubmitAction(): InfoBoxActionInterface {
    ...
    if (this.leftActions) {
      resultAction = this.leftActions.concat(this.rightActions).find((action: InfoBoxActionInterface) => action.isSubmit);
    }
    return resultAction;
  }
