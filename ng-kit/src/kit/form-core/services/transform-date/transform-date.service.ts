...
export class TransformDateService {
  ...
  parse(value: string, dateFormat: string): Date | null {
    // NOTE: Used UTC parser here, because `parseZone()` not working properly -
    ...
    let parsed: Moment = utc(value, dateFormat, true);
    if (!parsed || !parsed.isValid()) {
      parsed = this.parseSpecialFormats(value, dateFormat) || this.parseDateStringNative(value);
    }
    return parsed && parsed.isValid() ? this.momentToDate(parsed) : null;
  }

  private momentToDate(parsed: Moment): Date {
    return new Date(parsed.year(), parsed.month(), parsed.date());
  }

  private parseDateStringNative(value: string): Moment {
  ...
  private parseSpecialFormats(value: string, dateFormat: string): Moment {
    const normalizedValue: string = this.normalizeValue(value);

    let parsed: Moment;
    for (const format of this.getFormats(dateFormat)) {
      parsed = utc(normalizedValue, format, true);
      if (parsed && parsed.isValid()) {
        return parsed;
      }
    }
  ...
  private normalizeValue(value: string): string {
    let numbers: string = (value || '')
      ...
      .replace(/\s{2,}/g, ' ')
      .trim();
    if (numbers.length === 6 && numbers.substr(1, 1) === ' ') {
