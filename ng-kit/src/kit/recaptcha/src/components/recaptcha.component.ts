...
export class ReCaptchaComponent implements AfterViewInit, OnChanges {
  ...
  @Input() size: ReCaptchaSizeEnum = ReCaptchaSizeEnum.Normal;
  ...
  readonly defaultKey = settings.API_KEY;
      ...
      if ( grecaptcha ) {
        ...
        if (this.captchaRef2 && this.captchaRef2.nativeElement) {
          this.reCaptchaId = grecaptcha.render(this.captchaRef2.nativeElement, {
            sitekey: this.key || this.configService.getConfigConfig().recaptchaSiteKey || this.defaultKey,
