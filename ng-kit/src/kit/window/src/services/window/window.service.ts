...
import { DeviceType, WindowScrollInterface, WindowSizeInterface } from '../../interfaces';
...
export class WindowService implements OnDestroy {
  ...
  documentClickEvent$: Observable<Event> = fromEvent(this.window.document, 'click')
    .pipe(
   ...
   */
  messageEvent$: Observable<Event> = this.windowEventsService.message$(this.destroy$)
    .pipe(
      takeUntil(this.destroy$),
      startWith(void 0), // always start from last event
  ...
  constructor(
    // private window: Window, // non-injectable, use factory. Removed because doesn't work in ngc
    private windowEventsService: WindowEventsService,
    private platformId: string,
    // NOTE: usually device type can be injected from builder client for SSR
    @Inject(DEVICE_TYPE) @Optional() private deviceType: DeviceType
  ) {
    this.parseSizes();
  ...
  private parseSizes(): void {
    this.widthMobile = parseFloat(peVariables['viewportBreakpointSm2'] as string);
    this.widthIpad = parseFloat(peVariables['viewportBreakpointIpadPro'] as string);
    this.widthTablet = parseFloat(peVariables['viewportBreakpointSm3'] as string);
    this.widthDesktop = parseFloat(peVariables['viewportBreakpointMd1'] as string);
  }
      ...
      .pipe(
        ...
        map((data: boolean[]) => {
          switch (data.indexOf(true)) {
            ...
            case 2: return DeviceType.Desktop;
            case 3: return DeviceType.DesktopLg;
  ...
  private getWindowScrollData(): WindowScrollInterface {
    const scrollingElement: Element = (
      this.window && this.window.document && this.window.document.scrollingElement ?
