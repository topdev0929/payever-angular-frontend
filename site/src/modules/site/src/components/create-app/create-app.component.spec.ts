...
import { ConfirmDialogService } from '../confirm-dialog/dialog-data.service';
...
describe('PeSettingsCreateAppComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    confirmDialog = jasmine.createSpyObj<ConfirmDialogService>('ConfirmDialogService', ['open', 'onConfirmClick']);
  ...
  it('should set config on construct - appData.id is defined', () => {
    ...
    expect(envService.siteId).toEqual(appData.id);
  ...
  it('should remove site', () => {
    ...
    confirmDialog.onConfirmClick.and.returnValue(of(true));

    component.removeSite();

    expect(confirmDialog.open).toHaveBeenCalled();
    expect(confirmDialog.onConfirmClick).toHaveBeenCalled();
