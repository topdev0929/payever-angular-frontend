...
export class ActualPosApi implements PosApi {
  ...
  getIntegrationInfo(businessId: string, integration: string): Observable<IntegrationInfoInterface> {
    return this.http.get<IntegrationInfoInterface>(`${this.posApiPath}/business/${businessId}/integration/${integration}`);
  }

  getConnectIntegrationInfo(integrationId: string): Observable<IntegrationConnectInfoInterface> {
    return this.http.get<IntegrationConnectInfoInterface>(`${this.env.backend.connect}/api/integration/${integrationId}`);
  }
  ...
  toggleTerminalIntegration(businessId: string, terminalId: string, integrationName: string, enable: boolean) {
    return this.http.patch(`${this.posApiPath}/business/${businessId}/terminal/${terminalId}/integration/${integrationName}/${enable ? 'install' : 'uninstall'}`, {});
  }
}
