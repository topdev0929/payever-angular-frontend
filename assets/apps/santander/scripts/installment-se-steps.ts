...
const stepClasses: StepClasses[] = [
  ...
  { step: 'household-expenses-step', header: 'household-expenses-header-header' },
  { step: 'household-expenses-step', header: 'household-expenses-header-header' },
];

export const santanderInstallmentSEPaymentSteps = (step: Step): void => {
  const containerClasses: string[] = [
    ...
    "active-application-details-step",
    "active-household-expenses-step-1",
    "active-household-expenses-step-2",
  ];
  ...
  stepClasses.forEach(({ step: stepClass, header: headerClass }, index) => {
    ...
    const isHouseholdStep = stepClass === "household-expenses-step" && step >= 6 && step <= 7;

    toggleClass("main-container", containerClasses[index], isActive);

     if ((isPaymentMethodStep || isHouseholdStep) && hasClass(stepClass, "active")) {
      return;
    }
  ...
  const applicationDetailsStepDynamicHeigh: boolean = step >= 6;
  ...
  toggleClass("application-details-step-header", "dynamic-height", applicationDetailsStepDynamicHeigh);
...
export const santanderInstallmentSEIndustryHandler = () => {
