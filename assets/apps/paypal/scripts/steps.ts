...
export const payPalPaymentSteps = (step: Step): void => {
  ...
  stepClasses.forEach(({ step: stepClass, header: headerClass }, index) => {
    ...
    const showActivePaymentMethodStep: boolean = step >= 3;
    ...
    toggleClass("account-step-header", "dynamic-height", step >= 2);
    toggleClass("shipping-step-header", "dynamic-height", step >= 3);
    toggleClass("checkout-items", "with-offset", showActivePaymentMethodStep);
  });
...
const updateElementsVisibility = (step: Step): void => {
  ...
  const showModalBackdrop: boolean = step >= 4;
  ...
  showElement("outer-frame", false);

  const delay = delayedExecution(250);
  delay(() => {
    showElement("modal-backdrop", showModalBackdrop);
    toggleClass("modal-backdrop", "solid-light", step !== 5);
    showElement("outer-frame", step === 4);
