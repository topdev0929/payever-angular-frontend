...
import {
  ...
  CheckoutConnectionInterface, BusinessNotificationSettingsInterface, SendingMethodEnum, ErrorNotificationTypesEnum, CronUpdateIntervalEnum
} from '../interfaces';
import { PaymentOptionsInterface, SantanderDkProductInterface } from '@pe/finexp-app/finexp-editor/src/interfaces';
...
export class ApiService implements FinexpApiAbstractService {
  ...
  patchChannelSet(businessId: string, channelSetId: string, policyEnabled: boolean): Observable<void> {
    // tslint:disable:max-line-length
    return this.http.patch<void>(`${this.env.backend.checkout}/api/business/${businessId}/channelSet/${channelSetId}`, {policyEnabled: policyEnabled});
  }
  ...
  patchCheckoutSettings(businessId: string, settings: BusinessNotificationSettingsInterface[]): Observable<void> {
    return this.http.patch<void>(`${this.env.backend['errorNotifications']}/api/business/${businessId}/settings`, settings);
  }

  getCheckoutSettings(businessId: string): Observable<BusinessNotificationSettingsInterface[]> {
    return this.http.get<BusinessNotificationSettingsInterface[]>(`${this.env.backend['errorNotifications']}/api/business/${businessId}/settings`);
  }

  // saveCheckoutSettings(businessId: string, checkoutId: string, data: CheckoutSettingsInterface): Observable<CheckoutInterface> {
  //   return this.http.post<CheckoutInterface>(`${this.getConfig().checkout}/api/business/${businessId}/checkout/${checkoutId}/settings`, data);
  // }

  // putCheckout(checkoutId: string, data: CheckoutInterface): Observable<void> {
  //   return this.http.put<void>(`${this.getConfig().checkout}/api/new-checkout/${checkoutId}`, data);
  // }

  // updateCheckout(businessId: string, checkoutId: string, data: CheckoutInterface): Observable<void> {
  //   return this.http.post<void>(`${this.getConfig().checkout}/api/checkout/${businessId}/${checkoutId}/settings`, data);
  // }
  ...
  toggleCheckoutConnection(businessId: string, checkoutId: string, connectionId: string, enable: boolean): Observable<void> {
    return this.http.patch<void>(
