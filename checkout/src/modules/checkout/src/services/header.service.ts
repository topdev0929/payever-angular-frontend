...
export class HeaderService implements FinexpHeaderAbstractService { // extends AbstractService {
  ...
  setShortHeader(
    ...
    if (extraRightButtons && extraRightButtons.length) {
      ...
      lodashMap(extraRightButtons, button => {
        ...
        button.title = this.translateService.hasTranslation(button.title) ? this.translateService.translate(button.title) : button.title;
        button.onClick = () => {
  ...
  setShortHeaderWithDropdownMenu(titleKey: string, menuItems: PePlatformHeaderItem[] /*PePlatfromHeaderMenuItemControlInterface[]*/, onCancel: () => void, loading$: Observable<boolean> = null): void {
