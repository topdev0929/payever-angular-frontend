...
export class RootCheckoutWrapperService {
  ...
  private paramsSubject: BehaviorSubject<CheckoutStateParamsInterface> = new BehaviorSubject<CheckoutStateParamsInterface>(null);
...
*/
  doEmailPrefilled(): void {
    this.preparePrefilled(link => {
      this.storageService.getUserBusiness().subscribe(business => {
        ...
        this.renderer.setAttribute(a, 'href', `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`);
        a.click();
        a.remove();
      });
    });
  ...
  tryClipboardCopy(text: string): Observable<boolean> {
    ...
    this.snackBarService.show(this.translateService.translate('rootCheckoutWrapper.copied'));
    return of(true);
  }

  getCopyLink() {
    return this.storageService.makeCreateCheckoutLink(this.checkoutUuid, this.channelSetIdSubject.value);
  }

  private copyLink(): void {
    this.clipboardCopy(
      this.storageService.makeCreateCheckoutLink(this.checkoutUuid, this.channelSetIdSubject.value),
