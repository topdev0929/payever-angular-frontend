...
export class StorageService implements FinexpStorageAbstractService { // TODO Cleanup
  ...
  private businessInfoProcessed: boolean = false;
  ...
  fetchPhoneNumbers(reset: boolean = false): Observable<string[]> {
    const integrationName: string = 'twilio'; // TODO For now only Twilio but in future can be more
    return this.getInstalledConnections(reset).pipe(filter(d => !!d), take(1), flatMap(connections => {
      ...
      return !connection ? [] : this.apiService.fetchPhoneNumbersAction(this.businessUuid, connection._id, integrationName)
        .pipe(
