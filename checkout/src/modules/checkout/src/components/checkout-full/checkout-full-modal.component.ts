...
import { ApiService, EnvService, StorageService, RootCheckoutWrapperService } from '../../services';
...
export class CheckoutFullModalComponent extends AbstractComponent implements OnInit, OnDestroy { // TODO Rename as CheckoutAsModalComponent
    ...
    map((data: [CheckoutInterface[], CheckoutChannelSetInterface[], CheckoutInterface]) => {
        ...
        const newButton: SubdashboardHeaderDropdownItemInterface = {
          ...
          onClick: () => {
            this.storageService.setDefaultCheckout(this.checkoutUuid).subscribe(() => {
              this.snackBarService.show(this.translateService.translate('setCheckoutAsDefault.done'));
            }, error => {
              this.snackBarService.show(error ? error.message : this.translateService.translate('errors.unknown'));
            });
          }
        };
      ...
      if (checkoutList && checkoutList.length > 1 && this.isSwitchedEnabled) {
        const newButton: SubdashboardHeaderDropdownItemInterface = {
          ...
          onClick: () => this.router.navigate(['../../switch'], { relativeTo: this.activatedRoute }) // this.onClickButton('switch') // TODO
        };
  ...
  ngOnInit(): void {
    forkJoin(
      ...
      .subscribe(([business, currentCheckout]) => {
        this.customChannelList$ = this.storageService.getCategoryInstalledIntegrationsInfo(IntegrationCategory.Channels).pipe(
  ...
  navigateToCategory(panel: PanelInterface, matPanel: MatExpansionPanel): void {
    this.router.navigate(['..', matPanel.expanded ? `panel-${panel.key}` : 'view'], { relativeTo: this.activatedRoute, replaceUrl: true });
  }
   ...
   * @param channelSets
   */
  private isAllowToDeleteCheckout(checkoutList: CheckoutInterface[], channelSets: CheckoutChannelSetInterface[]): boolean {
    return !(
      checkoutList
      && checkoutList.length === 1
      && channelSets
      && channelSets.findIndex((channelSet: CheckoutChannelSetInterface) => this.defaultChannelSets.indexOf(channelSet.type) < 0) > -1
    );
  }
