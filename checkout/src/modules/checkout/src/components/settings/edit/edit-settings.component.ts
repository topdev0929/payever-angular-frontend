...
import { FormAbstractComponent, FormScheme, FormSchemeField, PeValidators, SnackBarService } from '@pe/forms';
...
export class EditSettingsComponent extends FormAbstractComponent<CheckoutInterface> implements OnInit {
  ...
  formStorageKey: string = 'checkout.edit';
  ...
  ngOnInit(): void {
    this.onSave$.pipe(filter((val) => !!val))
      .subscribe(() => {
        ...
        this.snackBarService.show('Not possible to save settings'); // TODO Translations
      });
  ...
  protected onSuccess(): void {
    ...
    this.storageServiceLocal.getCheckoutByIdOnce(this.checkoutUuid).subscribe(currentCheckout => {
      const newSettings: CheckoutSettingsInterface = Object.assign({}, currentCheckout.settings, this.form.getRawValue());
      this.storageServiceLocal.saveCheckoutSettings(currentCheckout._id, newSettings)
        ...
        (error) => {
          if (error.status === 400) {
            ...
            formField.setErrors({ external: this.translateService.translate('errors.phoneExists', { phoneNumber: formField.value }) });
