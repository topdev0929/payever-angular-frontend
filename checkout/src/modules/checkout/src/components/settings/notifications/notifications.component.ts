...
import {
  BusinessNotificationSettingsInterface, CronUpdateIntervalEnum, ErrorNotificationTypesEnum, IntegrationInfoInterface, TimeFramesInterface
} from '../../../interfaces';
...
export class NotificationsComponent extends AbstractComponent implements OnInit {
  ...
  formStorageKey: string = 'checkout-settings-notifications';
    ...
    if (this.businessSettings && !this.isSaving$.value) {
      ...
      this.checkoutStorageService.saveCheckoutNotificationSettings(settingsToSave).subscribe(() => {
        ...
        if (errMessages.length > 0) {
          this.snackBarService.show(errMessages.join(',<br>'));
        } else {
          // TODO Translations
          this.snackBarService.show(error.code === 400 ? 'Not possible to pass backend validation' : 'Not possible to save data');
        }
        this.isSaving$.next(false);
      });
    }
  ...
  private prepareGroupedSettings(): void {
    ...
    const integrationKeys = uniq(this.businessSettings.map(a => a.integration || null).filter(a => !a || availableIntegrations.indexOf(a) >= 0));
  ...
  private getRepeatFrequencyIntervalOptions(): SettingsOptionsInterface[] {
    ...
    for (let i in values) {
      options.push({
        value: values[i],
        label: this.translateService.translate(`settings.notifications.settings.repeatFrequencyInterval.options.${values[i]}`)
      });
    }
    return options;
  }
