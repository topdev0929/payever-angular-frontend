...
export class CronIntervalSettingsComponent extends AbstractComponent implements OnInit {
  ...
  ngOnInit() {
    this.form = this.formBuilder.group({
      ...
      sendEmailAfterInterval: [this.timeFrame.sendEmailAfterInterval, [Validators.required, Validators.min(0), Validators.max(720)]],
      repeatFrequencyInterval: [this.timeFrame.repeatFrequencyInterval, [Validators.required, Validators.min(0), Validators.max(60)]],
    });
  ...
  private getSendEmailAfterIntervalOptions(): SettingsOptionsInterface[] {
    ...
    for (let i in values) {
      options.push({
        value: values[i],
        label: this.translateService.translate(`settings.notifications.settings.sendEmailAfterInterval.options.${values[i]}`)
      });
    }
    return options;
  }
