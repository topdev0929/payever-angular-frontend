import { Component, Compiler, OnInit,
  AfterViewInit, OnChanges, ChangeDetectorRef,
  Injector, SimpleChanges, ViewChild, ViewContainerRef, Directive } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';

import { InstallPaymentsListComponent } from '@pe/connect-app';
...
export class WelcomePaymentsComponent extends AbstractComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    /*
    import('@pe/connect-app').then(({SharedModule}) => {
      this.compiler.compileModuleAsync(SharedModule).then(moduleFactory => {
        ...
        instance.onInstalledCount.pipe(takeUntil(this.destroyed$)).subscribe($event => this.onInstalledCountChanged($event));
