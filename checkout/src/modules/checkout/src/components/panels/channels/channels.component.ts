...
export class PanelChannelsComponent extends AbstractPanelComponent {
  ...
  channels$ = combineLatest(
    ...
    map(([applications, shopSystems, messaging]) => {
      return [].concat(
        applications?.filter(item => !Object.values(NotAllowedChannels).find(val => val === item.integration.name)) || [],
        shopSystems || [],
        messaging || []
      );
    }),
  ...
  customChannelsReady: boolean = true;
  ...
  ngOnInit(): void {
    ...
    this.storageService.getCheckoutByIdOnce(this.checkoutUuid).subscribe(currentCheckout => {
      this.storageService.getChannelSets().pipe(takeUntil(this.destroyed$), filter(d => !!d)).subscribe(channelSets => {
