...
const welcomeStepsEnabled: boolean = false;
    ...
    this.storageService.getUserBusiness().subscribe(business => {
      this.storageService.addNewCheckout({
        ...
        .subscribe((checkout: CheckoutInterface) => {
          // this.router.navigate([`${this.navigateBasement(route, businessUuid)}/${checkout._id}/view`], { replaceUrl: true });
          this.navigateToWelcomeScreen(route, businessUuid, checkout._id, null);
        });
    });
  }

  protected abstract navigateToWelcomeScreen(route: ActivatedRouteSnapshot, businessUuid: string, checkoutId: string, step: WelcomeStepEnum): void;

  protected abstract navigateToCheckoutHome(route: ActivatedRouteSnapshot, businessUuid: string, checkoutId: string): void;
