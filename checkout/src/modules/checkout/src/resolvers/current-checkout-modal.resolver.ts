...
export class CurrentCheckoutModalResolver extends BaseCurrentCheckoutResolver {
  ...
  protected navigateToWelcomeScreen(route: ActivatedRouteSnapshot, businessUuid: string, checkoutId: string, step: WelcomeStepEnum): void {
  ...
  protected navigateToCheckoutHome(route: ActivatedRouteSnapshot, businessUuid: string, checkoutId: string): void {
    ...
    this.router.navigate([`${this.navigateBasement(route, businessUuid)}/${checkoutId}/${routeName}`], { replaceUrl: true });
  }
