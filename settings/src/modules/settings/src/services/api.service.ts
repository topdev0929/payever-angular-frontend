...
import { UserAccountInterface } from '../misc/interfaces/user-account.interface';
...
export class ApiService {
  ...
  getBusiness(businessId: string): Observable<BusinessInterface> {
    const config = this.configService.getBackendConfig();
    const url: string = `${config.users}/api/business/${businessId}`;
  ...
  createBusinessEmployeeGroup(businessId: string, group: NewBusinessEmployeesGroupInterface): Observable<BusinessEmployeesGroupInterface> {
    const config = this.configService.getBackendConfig();
    const url = `${config.users}/api/employee-groups/${businessId}`;
  ...
  updateBusinessEmployeeGroup(businessId: string, group: NewBusinessEmployeesGroupInterface, groupId: string): Observable<BusinessEmployeesGroupInterface> {
    const config = this.configService.getBackendConfig();
  ...
  createBusinessEmployeeInGroup(businessId: string, groupId: string, employees: string[]): Observable<IGroupItemInterface> {
    const config = this.configService.getBackendConfig();
  ...
  getBusinessEmployeeAcls(businessId: string, employeeId: string): Observable<any> {
    const config = this.configService.getBackendConfig();
    const url = `${config.auth}/api/employees/get-acls/${employeeId}`;
  ...
  createBusinessEmployee(businessId: string, newEmployee: NewBusinessEmployeeInterface): Observable<BusinessEmployeeInterface> {
    const config = this.configService.getBackendConfig();
    const url = `${config.users}/api/employees/${businessId}`;
  ...
  updateBusinessEmployee(businessId: string, employeeId: string, employee: NewBusinessEmployeeInterface): Observable<BusinessEmployeeInterface> {
    const config = this.configService.getBackendConfig();
  ...
  getWallpapersByCode(code: string, page, limit): Observable<WallpaperDataInterface[]> {
    ...
    return this.http.get<WallpaperDataInterface[]>(`${config.wallpapers}/api/products/wallpapers/byId/${code}`, { params });
  }
  ...
  getBusinessProductWithIndustriesList(): Observable<ProductWithIndustriesInterface[]> {
    const config = this.configService.getBackendConfig();
    const url: string = `${config.commerceos}/api/business-products`;
    // TODO his pipe does not make sense.
        ...
        if (errorList) {
          ...
          if (errorList instanceof Array) {
            errorList.forEach((paramError: any) => {
              const errorText = Object.keys(paramError.constraints)
