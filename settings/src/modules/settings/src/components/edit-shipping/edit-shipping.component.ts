...
export class EditShippingComponent extends AbstractComponent implements OnInit {
  ...
  setAddressValue(form) {
    this.addressArray.push(`${form.get('street').value}, ${form.get('zipCode').value || ''}, ${form.get('city').value}, ${this.countries.find(val => val.value.toLowerCase() === form.get('country').value.toLowerCase())?.label || ''}`);
  }
