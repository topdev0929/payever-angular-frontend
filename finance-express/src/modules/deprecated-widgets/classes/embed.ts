...
function isDevServer(): boolean {
  return !!process.env.DEV_SERVER || window['pe_finExp_deprecatedWidget_devServer'];
}

function getDevDomain(root: string, domain: string): string {
  if (isDevServer()) {
    try {
      const envConfig = require('../../../sandbox/env.json');
...
const webWidgetsUrl = getDevDomain('backend', 'webWidgets') || 'MICRO_URL_WEB_WIDGETS';
const checkoutMicroVer: string = isDevServer() ? 'latest' : 'MICRO_CHECKOUT_VERSION';
...
export class Embed implements EmbedInterface {
  ...
  static fetchConfigAndRun() {
    ...
    FinanceExpressConfig.setConfig({
      ...
      envBaseUrl: `${checkoutWrapperUrl}/wrapper/${checkoutMicroVer}`,
      checkoutWrapperMicro: `${checkoutWrapperUrl}/wrapper/${checkoutMicroVer}/checkout-main-ce/micro.js`,
      commerceosOrigin: commerceosMicroUrl,
      commerceosWrapperStyles: `${commerceosMicroUrl}/lazy-styles.css`,
      checkoutWrapperStyles: `${checkoutWrapperUrl}/wrapper/${checkoutMicroVer}/lazy-styles.css`,
  ...
  collectWidgets(parentElement?: HTMLDocument) {
    ...
    for (let i = 0, element: Element; element = parentElement.querySelectorAll('.payever-finance-express')[i] as any; i++) {
      ...
      const fixNumber = (value: any): string => {
      ...
      const caclulationRequest: FinExpRequest = new FinExpRequest(
        (calculationResponse: CalculationResponseInterface) => {
          ...
          if (Embed.prototype.isNo) {
            calculationResponse.credit = calculationResponse.credit.filter(a => !!(a as any).monthlyAmount);
            const last: any = calculationResponse.credit[calculationResponse.credit.length - 1];
            const translateNO = (rate: any, text: string) => {
              return text
                .replace('%PRICE%', 'kr ' + fixNumber(data.price))
                .replace('%AMOUNT%', 'kr ' + fixNumber(rate.monthlyAmount))
                .replace('%RATE%', fixNumber(rate.effectiveRate))
                .replace('%MONTHS%', rate.duration)
                .replace('%TOTAL%', 'kr ' + fixNumber(rate.creditPurchase))
                .replace('%CREDIT_COST%', 'kr ' + fixNumber(rate.creditPurchase - Number(data.price)));
            }
            calcData.data = {
              credit: calculationResponse.credit.map((a: any) => {
                return {
                  campaign_code: a.campaignCode,
                  title: a.title,
                  description: a.description,
                  is_fixed_amount: a.isFixedAmount,
                  monthly_amount: a.monthlyAmount,
                  duration: a.duration,
                  credit_purchase: a.creditPurchase,
                  effective_rate: a.effectiveRate
                }
              }),
              "messages":{
                "banner_and_rate":{
                  "month_rate": translateNO(last, '%AMOUNT%'), // "santander.no.installment.finance_express.banner.month_rate"),
                  "duration": translateNO(last, '%MONTHS% måneder'), // "santander.no.installment.finance_express.banner.duration"),
                  "pay_in": translateNO(last, 'Betale i %MONTHS%&nbsp; måneder'), // santander.no.installment.finance_express.banner.pay_in"),
                  "deferral_change":null,
                  "summary": translateNO(last, 'Eff.rate %RATE%%, %PRICE% <br>per %MONTHS% må, total %TOTAL%'), // santander.no.installment.finance_express.banner.summary")
                },
                "link":{
                  "text": translateNO(last, 'Get it now from %AMOUNT%'), // santander.no.installment.finance_express.link.text"),
                  "effective_rate": translateNO(last, 'Effektiv rente'), // santander.no.installment.finance_express.link.effective_rate")
                },
                "button":{
                  "text": translateNO(last, 'Få det nå for %AMOUNT% i måneden'), // santander.no.installment.finance_express.button.text"),
                  "effective_rate": translateNO(last, 'Effektiv rente'), // santander.no.installment.finance_express.button.effective_rate")
                },
                "bubble": translateNO(last, 'Hi! Would you prefer to pay %AMOUNT%'), // santander.no.installment.finance_express.bubble"),
                "banner_and_rate_split": calculationResponse.credit.map((a: any) => {
                  return {
                    "banner_month_rate": translateNO(a, '%AMOUNT%'), // santander.no.installment.finance_express.banner.month_rate"),
                    "banner_duration": translateNO(a, '%MONTHS% måneder'), // santander.no.installment.finance_express.banner.duration"),
                    "banner_pay_in": translateNO(a, 'Betale i %MONTHS%&nbsp; måneder'), // santander.no.installment.finance_express.banner.pay_in"),
                    "banner_deferral_charge": translateNO(a, 'Utsettelse kostnad <br> %AMOUNT%'), // santander.no.installment.finance_express.banner.deferral_charge"),
                    "banner_summary": translateNO(a, 'Eff.rate %RATE%%, %PRICE% <br>per %MONTHS% må, total %TOTAL%'), // santander.no.installment.finance_express.banner.summary"),
                    "banner_total": translateNO(a, 'Totalt %TOTAL%'), // santander.no.installment.finance_express.banner.total"),
                    "banner_legal_info_text": translateNO(a, ''), // santander.no.installment.finance_express.banner.legal"),
                    "banner_effective_rate": translateNO(a, 'eff.rente %RATE%%'), // santander.no.installment.finance_express.banner.effective_rate"),
                    "banner_credit_cost": translateNO(a, 'kostnad %CREDIT_COST% <br> Delbetaling forutsetter inngåelse av kredittavtale med Santander med kredittkort etter en kredittvurdering'), // santander.no.installment.finance_express.banner.credit_purchase"),
                    "calculator_month_rate": translateNO(a, '%AMOUNT%'), // santander.no.installment.finance_express.calculator.month_rate"),
                    "calculator_duration": translateNO(a, '%MONTHS% måneder'), // santander.no.installment.finance_express.calculator.duration"),
                    "calculator_pay_in": translateNO(a, 'Betale i %MONTHS%&nbsp; måneder'), // santander.no.installment.finance_express.calculator.pay_in"),
                    "calculator_deferral_charge": translateNO(a, 'Utsettelse kostnad <br> %AMOUNT%'), // santander.no.installment.finance_express.calculator.deferral_charge"),
                    "calculator_summary": translateNO(a, 'Eff.rate %RATE%%, %PRICE% <br>per %MONTHS% må, total %TOTAL%'), // santander.no.installment.finance_express.calculator.summary"),
                    "calculator_total": translateNO(a, 'Totalt %TOTAL%'), // santander.no.installment.finance_express.calculator.total"),
                    "calculator_legal_info_text": translateNO(a, 'Juridisk informasjon'), // santander.no.installment.finance_express.calculator.legal"),
                    "calculator_effective_rate": translateNO(a, 'eff.rente %RATE%%'), // santander.no.installment.finance_express.calculator.effective_rate"),
                    "calculator_credit_cost": translateNO(a, 'kostnad %CREDIT_COST% <br> Delbetaling forutsetter inngåelse av kredittavtale med Santander med kredittkort etter en kredittvurdering'), // santander.no.installment.finance_express.calculator.credit_purchase")
                  }
                })
              }
            } as any;
          ...
          if (Embed.prototype.isDk) {
            // calculationResponse.credit = calculationResponse.credit.filter(a => !!(a as any).monthlyAmount);
            const last: any = calculationResponse.credit[calculationResponse.credit.length - 1];
            const translateDK = (rate: any, text: string) => {
              return text
                .replace('%AMOUNT%', 'kr ' + fixNumber(rate.result.monthlyPayment))
                .replace('%RATE%', fixNumber(rate.result.annuallyProcent))
                .replace('%INTEREST_RATE%', fixNumber(rate.parameters.nominalInterest))
                .replace('%PRICE%', 'kr ' + fixNumber(rate.result.totalCost))
                .replace('%MONTHS%', rate.result.termsInMonth)
                .replace('%TOTAL%', 'kr ' + fixNumber(rate.result.totalLoanAmount))
                .replace('%FEE%', 'kr ' + fixNumber(rate.result.totalLoanAmount - rate.result.totalCost));
            }
            calcData.data = {
              credit: calculationResponse.credit.map((a: any) => {
                return {
                  Parameters: {
                    LoanAmount: a.parameters.loanAmount,
                    EstablishmentFee: a.parameters.establishmentFee,
                    LoanDurationInMonths: a.parameters.loanDurationInMonths,
                    NominalInterest: a.parameters.nominalInterest,
                    EffectiveInterest: a.parameters.effectiveInterest,
                    MonthlyAdministrationFee: a.parameters.monthlyAdministrationFee,
                    PaymentFreeDuration: a.parameters.paymentFreeDuration,
                    PaymentFreeIntrest: a.parameters.paymentFreeIntrest,
                    PaymentFreePayInstallments: a.parameters.paymentFreePayInstallments,
                    startDate: null,
                  },
                  Result: {
                    TermsInMonth: a.result.termsInMonth,
                    AnnuallyProcent: a.result.annuallyProcent,
                    TotalCost: a.result.totalCost,
                    TotalLoanAmount: a.result.totalLoanAmount,
                    MonthlyPayment: a.result.monthlyPayment,
                    PaymentFreeDuration: a.result.paymentFreeDuration,
                  },
                  isDefault: a.isDefault,
                  payLaterType: a.payLaterType,
                  interestFreeType: a.interestFreeType
                }
              }),
              "messages":{
                "banner_and_rate":{
                  "month_rate": translateDK(last, '%AMOUNT%'), // "santander.dk.installment.finance_express.banner.month_rate"),
                  "duration": translateDK(last, '%MONTHS% måneder'), // "santander.dk.installment.finance_express.banner.duration"),
                  "pay_in": translateDK(last, '%AMOUNT% pr. md. i %MONTHS% mdr.'), // santander.dk.installment.finance_express.banner.pay_in"),
                  "deferral_change":null,
                  "summary": translateDK(last, 'Eff. rente %RATE%%, %PRICE%<br>pr. %MONTHS% md, samlet %TOTAL%'), // santander.dk.installment.finance_express.banner.summary")
                },
                "link":{
                  "text": translateDK(last, 'Få det nu fra %AMOUNT%'), // santander.dk.installment.finance_express.link.text"),
                  "effective_rate": translateDK(last, 'Effective rate %RATE%%'), // santander.dk.installment.finance_express.link.effective_rate")
                },
                "button":{
                  "text": translateDK(last, 'Få det nu fra %AMOUNT%/md'), // santander.dk.installment.finance_express.button.text"),
                  "effective_rate": translateDK(last, 'Eff.sats %RATE%%, %PRICE% pr %MONTHS% mo, i alt %TOTAL%'), // santander.dk.installment.finance_express.button.effective_rate")
                },
                "bubble": translateDK(last, 'Hej - ville du foretrække at betale <strong>%AMOUNT%</strong>?'), // santander.dk.installment.finance_express.bubble"),
                "banner_and_rate_split": calculationResponse.credit.map((a: any) => {
                  return {
                    "banner_month_rate": translateDK(a, '%AMOUNT%'), // santander.dk.installment.finance_express.banner.month_rate"),
                    "banner_duration": translateDK(a, '%MONTHS% måneder'), // santander.dk.installment.finance_express.banner.duration"),
                    "banner_pay_in": translateDK(a, '%AMOUNT% pr. md. i %MONTHS% mdr.'), // santander.dk.installment.finance_express.banner.pay_in"),
                    "banner_deferral_charge": translateDK(a, 'Samlede omkostninger %AMOUNT%'), // santander.dk.installment.finance_express.banner.deferral_charge"),
                    "banner_summary": translateDK(a, 'Eff. rente %RATE%%, %PRICE%<br>pr. %MONTHS% md, samlet %TOTAL%'), // santander.dk.installment.finance_express.banner.summary"),
                    "banner_total": translateDK(a, 'Samlet kreditbeløb %FEE% Ydelse pr. måned %AMOUNT% i %MONTHS% måneder. Samlede kreditomkostninger %PRICE% Samlet tilbagebetalingsbeløb %TOTAL% Debitorrente %INTEREST_RATE%% (fast) ÅOP %RATE%% Forudsat betaling via betalingsservice. Fortrydelsesret 14 dage. Finansiering via Santander Consumer Bank.'), // santander.dk.installment.finance_express.banner.total"),
                    "banner_legal_info_text": translateDK(a, 'Samlet kreditbeløb %FEE% Ydelse pr. måned %AMOUNT% i %MONTHS% måneder. Samlede kreditomkostninger %PRICE% Samlet tilbagebetalingsbeløb %TOTAL% Debitorrente %INTEREST_RATE%% (fast) ÅOP %RATE%% Forudsat betaling via betalingsservice. Fortrydelsesret 14 dage. Finansiering via Santander Consumer Bank.'), // santander.dk.installment.finance_express.banner.legal"),
                    "banner_effective_rate": translateDK(a, ''), // santander.dk.installment.finance_express.banner.effective_rate"),
                    "banner_credit_cost": translateDK(a, ''), // santander.dk.installment.finance_express.banner.credit_purchase"),
                    "calculator_month_rate": translateDK(a, '%AMOUNT%'), // santander.dk.installment.finance_express.calculator.month_rate"),
                    "calculator_duration": translateDK(a, '%MONTHS% måneder'), // santander.dk.installment.finance_express.calculator.duration"),
                    "calculator_pay_in": translateDK(a, '%AMOUNT% pr. md. i %MONTHS% mdr.'), // santander.dk.installment.finance_express.calculator.pay_in"),
                    "calculator_deferral_charge": translateDK(a, 'Samlede omkostninger %AMOUNT%'), // santander.dk.installment.finance_express.calculator.deferral_charge"),
                    "calculator_summary": translateDK(a, 'Samlet kreditbeløb %FEE% Ydelse pr. måned %AMOUNT% i %MONTHS% måneder. Samlede kreditomkostninger %PRICE% Samlet tilbagebetalingsbeløb %TOTAL% Debitorrente %INTEREST_RATE%% (fast) ÅOP %RATE%% Forudsat betaling via betalingsservice. Fortrydelsesret 14 dage. Finansiering via Santander Consumer Bank.'), // santander.dk.installment.finance_express.calculator.summary"),
                    "calculator_total": translateDK(a, 'Samlet kreditbeløb %FEE% Ydelse pr. måned %AMOUNT% i %MONTHS% måneder. Samlede kreditomkostninger %PRICE% Samlet tilbagebetalingsbeløb %TOTAL% Debitorrente %INTEREST_RATE%% (fast) ÅOP %RATE%% Forudsat betaling via betalingsservice. Fortrydelsesret 14 dage. Finansiering via Santander Consumer Bank.'), // santander.dk.installment.finance_express.calculator.total"),
                    "calculator_legal_info_text": translateDK(a, 'Samlet kreditbeløb %FEE% Ydelse pr. måned %AMOUNT% i %MONTHS% måneder. Samlede kreditomkostninger %PRICE% Samlet tilbagebetalingsbeløb %TOTAL% Debitorrente %INTEREST_RATE%% (fast) ÅOP %RATE%% Forudsat betaling via betalingsservice. Fortrydelsesret 14 dage. Finansiering via Santander Consumer Bank.'), // santander.dk.installment.finance_express.calculator.legal"),
                    "calculator_effective_rate": translateDK(a, 'Effective rate %RATE%%'), // santander.dk.installment.finance_express.calculator.effective_rate"),
                    "calculator_credit_cost": translateDK(a, 'Credit cost %CREDIT_COST%'), // santander.dk.installment.finance_express.calculator.credit_purchase")
                  }
                })
              },
            } as any;
      ...
      const run = () => {
        ...
        new FinExpRequest(
          (response: any) => {
            ...
            if (Embed.prototype.isNo || Embed.prototype.isDk) {
              const business = FinanceExpressConfig.getConfig().businessSlug;
              caclulationRequest.post(
                `${webWidgetsUrl}/api/app/finance-express/business/${business}/client-action/calculate-rates`,
                {
                  amount: String(data.price),
                  paymentOption: Embed.prototype.isNo ? 'santander_installment_no' : 'santander_installment_dk',
                  widgetType: 'text',
