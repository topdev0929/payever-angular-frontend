...
export class PeChatHeaderComponent implements OnInit {
  ...
  get members(){
    return this.peMessageAppService.selectedChannel.members.length;
  }

  get liveMembersCount(){
    return this.peMessageAppService.selectedChannel.onlineMembersCount;
  }
  ...
  chatHeaderModeToggleMethod() {
    this.peMessageAppService.showPinnedMessageList = true;
  }
  ...
  private filter(value: any): PeChatAutocompleteOption[] {
  ...
  clickPinnedMessage(message: PeChatMessage) {
    ...
    this.chatScrollService.scrollToMessage$.next(message);
  }
