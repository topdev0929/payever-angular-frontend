...
import { PeMessageApiService } from '@pe/message/shared';
import {
  ...
  openRules(theme: string): void {
    forkJoin([
      ...
      this.apiService.getFolderTree().pipe(
        map((folders) => {
          return this.folderTreeFlatten(folders.filter((folder: any) => folder.scope !== 'default'));
        }),
      ),
    ]).pipe(
      tap(([values, rules, folders]) => {
        const { conditions, fields, actions, channels } = values as RuleValues;
        const onErrorSubject$ = new Subject<string>();
        ...
        this.ruleService.show(this.onSaveSubject$, data, onErrorSubject$);
      }),
      this.errorHandler(),
      takeUntil(this.destroyed$),
    ).subscribe();
