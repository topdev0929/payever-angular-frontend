...
import { ContextMenuConfig, PebContextMenuComponent, PE_CONTEXTMENU_DATA, PE_CONTEXTMENU_THEME } from '@pe/ui';

import {
  PeMessageChatContextMenuComponent,
} from '../components/message-chat-context-menu/message-chat-context-menu.component';
...
export class PeMessageChatContextMenuService {
  ...
  private attachDialogContainer(overlayRef: OverlayRef, config, dialogRef: PeContextMenuRef) {
    ...
    const containerPortal = new ComponentPortal(PeMessageChatContextMenuComponent, null, injector);
    ...
    containerRef.instance.select$.pipe(
      ...
      takeUntil(containerRef.instance.destroy$)).subscribe();
