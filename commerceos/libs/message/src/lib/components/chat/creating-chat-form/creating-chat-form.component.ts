...
import { PeMessageChatType, PeMessageCreatingChatData } from '@pe/message/shared';
...
export class PeCreatingChatFormComponent implements OnInit, AfterViewInit {
  ...
  setNextButtonText(loading: boolean | null, suffix = 'create'): void {
    const messageChatType = `message-integration.new-${this.messageChatType}`
    const titleTranslation = suffix === 'done'
      ? 'message-app.channel.settings.add-members'
      : `message-app.${messageChatType}`;

    !loading && (this.title = this.translateService.translate(titleTranslation));
