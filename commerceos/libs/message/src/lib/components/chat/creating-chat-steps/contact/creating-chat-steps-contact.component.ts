...
import { EnvironmentConfigInterface, EnvService, PE_ENV, PeDestroyService, PeGridItem } from '@pe/common';
...
export class PeCreatingChatStepsContactComponent implements OnInit, OnChanges {
  ...
  @Input() set animationTypeStepDoneTrigger(value: boolean) {
    if (value) {
      this.slugInputRef?.nativeElement?.focus();
    }
  }
    ...
    if (PeMessageChatSteep.Contacts === changes.step?.currentValue) {
      ...
      if (this.isPrivate && members.length > 0) {
        ...
        additionalSteep$ = this.peMessageApiService.patchChannel(
          ...
          tap((channel) => {
            this.peMessageAppService.addChannel(channel);
          }),
          switchMap(() => {
            if (members.length > 0) {
      ...
      additionalSteep$.pipe(
        ...
        catchError((error) => {
          ...
          if (error.message && error.key) {
            ...
            this.snackBarShow(error.error.message, false);
          }
      ...
      map((contacts: Contact[]) => {
        ...
        if (contacts.length === 0) {
          if (emailIsInvalid) {
            ...
            contactsNew = [this.serializeContact({ email: filter })];
          }
        } else {
          contactsNew = contacts.map((contact) => {
        ...
        return filterUnique.filter(
          user => !this.peMessageAppService.selectedChannel.members.some(
