...
export class PeMessageAppearanceComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    const initCurrSettings$ = this.peMessageIntegrationService.currSettings$.pipe(
      ...
      tap((settings: PeMessageIntegrationThemeItem) => {
        ...
        this.blurValue = settings.settings.messageWidgetBlurValue;
        this.alwaysOpen = settings.settings.alwaysOpen;
      }),
    );
    ...
    merge(initSettingsStream$, initCurrSettings$, currSettings$).pipe(
      tap(() => {
  ...
  selectMockUp(mockUp: PeMessageIntegrationThemeItem): void {
    this.mockUps.forEach((mItem: PeMessageIntegrationThemeItem) => {
      mItem.isDefault = false;
    });
    mockUp.isDefault = true;
    mockUp.settings.messageWidgetBlurValue = '';
    ...
    this.alwaysOpen = mockUp.settings?.alwaysOpen || false;
  ...
  changeOpenWidget(event: boolean): void {
    const currSettings = this.peMessageIntegrationService.currSettings;
    currSettings.settings.alwaysOpen = event;
    this.peMessageIntegrationService.currSettings = currSettings;
  }

  changeBlurMode(event: string): void {
    const currSetting = this.peMessageIntegrationService.currSettings;
    currSetting.settings.messageWidgetBlurValue = event;
    this.peMessageIntegrationService.currSettings = currSetting;
  }
