...
export class PebSelectComponent implements AfterContentInit {
  ...
  /** Sets select overlayMinWidth */
  @Input() public overlayMinWidth = 0;
  /** Sets select hideLabel */
  @Input() public hideLabel;
  /** Sets select arrowIcon */
  @Input() public arrowIcon = '#icon-ui-arrow-open';
  /** Sets select type */
  ...
  ngAfterContentInit() {
    this.selectedChanged$.subscribe((value) => {
      ...
      } else if (this.virtualOption.length !== 0) {
        if (this.multiple) {
          if (value) {
      ...
      } else if (this.optionGroup.toArray().length !== 0) {
    ...
    if ((this.preselected || this.preselected === 0) && this.options) {
  ...
  public showDropdown() {
    if (!this.disabled) {
      ...
      this.dialogRef = this.dropdown.show(width > this.overlayMinWidth
        ? width : this.overlayMinWidth);
      this.isFocused = true;
      this.cdr.detectChanges();
    }
  }
