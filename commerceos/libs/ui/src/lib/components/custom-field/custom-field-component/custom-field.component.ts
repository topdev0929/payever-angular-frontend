...
export class PeCutomFieldComponent implements OnInit {
  ...
  deleteAdditionalOptions(startIndex: number): void {
    ...
    while (index < this.defaultValues.value.length) {
      if (this.defaultValues.value[index].name === '') {
  ...
  onUpdateOptionsList($item: FieldTypesEnum): void {
    if (!this.hasOptions($item)) {
      return;
    }
    if ($item === FieldTypesEnum.RadioButton) {
      this.updateOptionList(2);

      return;
    }

    this.updateOptionList(1);
  }
  ...
  canDeleteOption() {
    if (this.newField.controls.type.value === FieldTypesEnum.RadioButton) {
      return this.defaultValues?.value?.length <= 2;
    } else {
      return this.defaultValues?.value?.length <= 1;
    }
  }
