...
import { OnboardingService } from '@pe/shared/onboarding';
import { TrafficSourceService } from '@pe/shared/traffic-service';
import { PeUser, StateUserService, UserState, LoadBusinesses, DefaultBusinessesLoaded } from '@pe/user';
...
export class AddBusinessOverlayComponent implements OnInit, OnDestroy {
  ...
  private addCompany(businessId: string) {
    ...
    this.apiService.createCompany(createCompanyData)
            ...
            this.enable(businessId).pipe(tap(() => {
              if (this.userData) {
                ...
                this.partnerService.getPartnerData({
                  ...
                  switchMap(partnerData => this.onboardingService.runAfterActions(
      ...
      this.onboardingService.actionController.pipe(takeUntil(this.destroyed$)).subscribe(() => {
