...
import { PeMessageSettings, PeMessageIntegrationThemeItem, PeMessageSubscription, PeMailConfig } from '../interfaces';
...
import {
  ...
  SetMailConfig,
  InitMessageSettings,
  SetCurrentMessageSettings, SetSubscriptionList, SetLiveChatBusinessId,
} from './message.actions';

const messageStateName = 'peMessageState';

export interface PeMessageState {
  liveChatBusinessId: string;
...
export class MessageState {
  @Selector()
  static liveChatBusinessId(state: PeMessageState): string {
    return state.liveChatBusinessId;
  }
  ...
  @Action(SetLiveChatBusinessId)
  setLiveChatBusinessId(ctx: StateContext<PeMessageState>, { businessId }: SetLiveChatBusinessId) {
    ctx.patchState({
      liveChatBusinessId: businessId,
    });
  }
}
