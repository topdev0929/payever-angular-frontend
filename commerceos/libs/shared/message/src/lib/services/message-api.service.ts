...
import { AuthHeadersEnum } from '@pe/auth';
// eslint-disable-next-line @nrwl/nx/enforce-module-boundaries
...
import {
  ...
  PeChatAttachFileViaUrl,
} from '@pe/shared/chat';
import { FolderItem, FolderPosition } from '@pe/shared/folders';
...
export class PeMessageApiService {
  ...
  public toggleChannelLiveStatus(
    channel: PeMessageChat,
    isLive: boolean,
    businessId: string = this.pebEnvService.businessId,
  ): Observable<any> {
    return this.httpClient.post(
      `${this.peMessageApiPath}/api/business/${businessId}/messaging/channel/${channel._id}/permissions`,
      { ...channel.permissions, live: isLive },
    );
  }
  ...
  public fetchGuestToken(hashData?: string) {
    const options = {
    ...
    return this.httpClient.post(`${this.env.backend.message}/api/public-channels/live-chat/guest-token`, { visitorHash: hashData }, options);
  }

  public fetchIpAddress(): Observable<{ ip: string }> {
  ...
  postFileViaURL(url: string): Observable<PeChatAttachFileViaUrl> {
    return this.httpClient.post<PeChatAttachFileViaUrl>(
      `${this.peMediaApiPath}/api/file/business/${this.businessId}/message/application/${this.applicationId}/via-url`,
      { url });
  }
  ...
  public getShop(): Observable<any> {
    return this.httpClient
      .get(`${this.env.backend.shop}/api/business/${this.businessId}/shop`, { headers: { [AuthHeadersEnum.allow403]: 'true' } })
      .pipe(catchError(() => of([])));
  }
