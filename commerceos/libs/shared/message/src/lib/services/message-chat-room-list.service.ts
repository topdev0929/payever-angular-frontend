...
import { PeMessageAppService } from '../utils';
...
export class PeMessageChatRoomListService {
  ...
  readonly closeFormReopenStream$ = new BehaviorSubject<boolean>(false);
  readonly activeChat$ = this.activeChatStream$.asObservable();

  public get activeChat(): PeMessageChat | null {
    return this.peMessageAppService.selectedChannel ?? null;
  }
  ...
  public readonly pendingToActivateChat$ = new Subject<PeMessageChat>();

  set pendingToActivateChat(chat: PeMessageChat) {
    this.pendingToActivateChat$.next(chat);
  }
    ...
    return chat.type === PeMessageChatType.AppChannel
      ...
      : this.peMessageApiService
        ...
        .pipe(
          ...
          tap(() => {
            this.overlayFormReopen = this.closeFormReopenStream$.value;
          }));
  }
  ...
  isUserInChatAlready(filterValue: string): boolean {
    return this.activeChat.members.some(
