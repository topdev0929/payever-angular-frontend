...
export class MessageStateService {
  ...
  private getConversation(id: string, type: PeMessageChatType, resetCache = false): Observable<any> {
    return this.conversation$.value?.[id] && !resetCache
      ...
      : of(type)
          .pipe(
            switchMap((chatType) => {
              if (chatType === PeMessageChatType.Group) {
            ...
            switchMap((res) => {
              ...
              conversationObj[id] = Array.isArray(res) ? { ...res[0], messages:[...res[0].messages] } : { ...res, messages:[...res.messages] };
  ...
  public messageDeleted(messageToDelete: PeChatMessage) {
    ...
    const removeMessage = (messages: PeChatMessage[]) => messages.filter(message => message._id !== messageToDelete._id);
