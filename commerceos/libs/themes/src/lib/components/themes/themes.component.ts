...
import { PebCreateEmptyThemeAction, PebCreateEmptyThemeSuccessAction } from "@pe/builder/actions";
import { PebEditorApi, PebWebsocketService } from '@pe/builder/api';
import {
  PebEnvService,
...
import { PeThemesGridService, ThemesApi } from '../../services';
...
@Component({
  ...
  providers: [PeDestroyService, PeOverlayWidgetService],
})
export class PeThemesComponent implements OnInit, OnDestroy {
  ...
  public itemContextMenu = {
    ...ITEM_CONTEXT_MENU, items: [...ITEM_CONTEXT_MENU.items, {
      label: 'grid.context_menu.download',
      value: PeGridContextMenuActionsEnum.Download,
    }],
  };
  ...
  private readonly initFoldersTree$ = of(this.env.id)
    .pipe(
      switchMap((applicationId) => {
        this.peFoldersApiService.applicationId$.next(applicationId);
        this.peFoldersApiService.hostPath$.next(this.env.builder + '/api');
    ...
    .pipe(
      tap((active: boolean) => {
        ...
        this.headerService.assignConfig(
          Object.assign({}, this.headerService.config, { isShowMainItem: this.isMobile && active },
        ));
      }));
  ...
  public get showAddNewItem(): boolean {
    return !this.isMobile && !this.selectedFolder?.isProtected;
  }
  ...
  public createByHand(): void {
    ...
    this.store.dispatch(new PebCreateEmptyThemeAction()).pipe(
      switchMap(() => this.actions$.pipe(
        ofActionDispatched(PebCreateEmptyThemeSuccessAction),
  ...
  changeJsonFileInput({ target }) {
    const files = target.files as FileList;

    if (files.length) {
      const selectedFolderId = this.startFolderId ?? this.rootFolder._id;

      this.themesApi.importTheme(files[0], selectedFolderId)
        .pipe(
          switchMap((response) => {
            const themeToMap = {
              isFolder: false,
            ...
            const themeId = mappedTheme[0].serviceEntityId;

            return this.themeResolver.resolveThemeById(themeId).pipe(
              tap(() => {
                const url = this.router.url.replace('themes', 'edit');

                this.router.navigate([url], { queryParams: {}, replaceUrl: true }).then(() => {
                  this.showSnackbar(notify, true);
                  this.headerService.removeSidenav(SIDENAV_NAME);
                });
              }),
  ...
  public actionClick(gridItem: PeGridItem, preview?: boolean): void {
    if (gridItem.type === PeGridItemType.Folder) {
      ...
      themeAction$.pipe(
        switchMap(() => this.themeResolver.resolveActiveTheme()),
        tap(() => {
          const url = this.router.url.replace('themes', 'edit');
          this.router.navigate([url], { queryParams: {}, replaceUrl: true }).then(() => {
            this.showSnackbar(notify, true);
  ...
  private downloadTheme(gridItem?: PeGridItem): void {
    if (gridItem.type === PeGridItemType.Folder) {
      this.snackbarService.toggle(true, {
        content: "Please select a theme to download",
        duration: 15000,
        iconColor: '#E2BB0B',
    ...
    this.themesApi.downloadTheme(gridItem.serviceEntityId, gridItem.title + ".json").subscribe();
  }
  ...
  public itemContextSelect(event: PeGridItemContextSelect): void {
    ...
    switch (action) {
      ...
      case PeGridContextMenuActionsEnum.AddFolder:
        this.createFolder();
        break;
      case PeGridContextMenuActionsEnum.Download:
        this.downloadTheme(gridItem);
        break;
    }
  }
