...
export class MediaService extends PebMediaService {
  private readonly sizeLimitation = 100 * 1024 * 1024;
  ...
  getCollection({ pagination = {}, filters = {} }: {
    ...
    Object.entries(filters).forEach(([key, value], i) => {
      ...
      } else if (value !== undefined && (typeof value !== 'object' || Object.keys(value).length)) {
    ...
    return this.http.get<PebMediaSidebarCollectionItem>(
      `https://studio-backend.test.devpayever.com/api/${this.businessId}/media`
    );
  }
  ...
  uploadImage(file: File, container: string, blobName?: string): Observable<any> {
    const validationError = this.checkFileValidation(file);
    if (validationError) {
    ...
    return this.http.post<any>(url, formData, { params }).pipe(
      map((response) => {
  ...
  private checkFileValidation(file: File): string | undefined {
    if (!this.fileSizeIsValid(file)) {
      return this.translateService.translate('builder-app.media.large_file_error');
    }

    if (!this.fileTypeIsValid(file)) {
      return this.translateService.translate('builder-app.media.file_type_error');
    }

    return undefined;
  }

  private fileSizeIsValid(file: File): boolean {
    if (file.size > this.sizeLimitation) {
      return false;
    }
  ...
  private fileTypeIsValid(file: File): boolean {
  ...
  uploadImageWithProgress(file: File, container: string) {
    ...
    const params = new HttpParams().set('skipResize', 'true');

    return this.http.post<ImageResponse>(
      `${this.mediaPath}/api/image/business/${this.businessId}/${container}`,
      formData, { params, reportProgress: true, observe: 'events' }).pipe(
      map((event) => {
        return {
