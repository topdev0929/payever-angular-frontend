...
function getDocumentRect(node: any): DOMRect {
  return node.nodeName === "PEB-RENDERER"
...
function getCorrectCoordinate(rects: any[], left: number, top: number): { x: number, y: number } {
  const max = {
    top: rects.reduce((prev, curr) => prev.top < curr.top ? prev : curr, 0).top,
    right: rects.reduce((prev, curr) => prev.right > curr.right ? prev : curr, 0).right,
    bottom: rects.reduce((prev, curr) => prev.bottom > curr.bottom ? prev : curr, 0).bottom,
    left: rects.reduce((prev, curr) => prev.left < curr.left ? prev : curr, 0).left,
  };
