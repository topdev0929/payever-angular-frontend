...
import { BehaviorSubject, combineLatest, EMPTY, fromEvent, merge, Observable, Subject } from 'rxjs';
...
export class RulesComponent extends BaseRules implements OnInit, AfterViewInit, OnDestroy {
  ...
  onCloseSubject$ = new BehaviorSubject(null);
  ...
  private addRuleOverlayRef: PeOverlayRef;
  ...
  onActionClick(action: ActionType, data?: RuleModel) {
    switch (action) {
      ...
      case ActionType.Duplicate:
        this.addRuleOverlayRef = this.openOverlay(data, action, this.overlayConfig.onError$);
        break;
  ...
  private openOverlay(ruleData?: RuleModel, action?: ActionType, onErrorSubject$?: Subject<string>) {
    const headerConfig = {
      onSaveSubject$: this.onSaveSubject$,
      onCloseSubject$: this.onCloseSubject$,
