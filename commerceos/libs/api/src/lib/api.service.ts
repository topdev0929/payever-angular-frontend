...
import { AuthHeadersEnum, PeAuthService } from '@pe/auth';
...
export class ApiService {
  ...
  getSpotlightSearch(pattern: string, limit: number, businessId: string): Observable<SpotlightResp> {
    ...
    .get<SpotlightResp>(url, { 
      headers: { [AuthHeadersEnum.allow403]: 'true' },
      params,
    }).pipe(
      map(data => data),
  ...
  transferBusinessOwnership(token: string): Observable<void> {
    const url = `${this.config.users}/api/business/transfer-ownership`;
  ...
  createCompany<T>(data: any): Observable<T> {
    const url = `${this.config.users}/api/business`;
