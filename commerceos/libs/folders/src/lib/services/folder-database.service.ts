...
import { DragAreaTypes, FolderItem } from '@pe/shared/folders';
...
export class FolderDatabase {
  ...
  duplicate(newItem: FolderItem) {
    const parent: FolderItem = newItem.parentFolderId ? this.getFolderById(newItem.parentFolderId) :  null;
  ...
  updateItem(node: FolderItem, name?: string) {
    ...
    const updateTree = (treeToUpdate: FolderItem[], nodeToUpdate: FolderItem): boolean => {
      if (!treeToUpdate) {

        return false;
      }

      treeToUpdate.some((node) => {
        if (node._id === nodeToUpdate._id) {
          node.name = name;
  ...
  getFolderById(id: string, list: FolderItem[] = this.data): FolderItem {
    for (const currentRoot of list) {
      if (currentRoot._id === id) {
        return currentRoot;
      }
      const folder = this.getFolderById(id, currentRoot.children);
      if (folder !== null) {
