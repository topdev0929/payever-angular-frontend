...
import { PeGridSearchDataInterface, PeGridSortingDirectionEnum, PeGridSortingOrderByEnum } from '@pe/grid/shared';
import { TranslateService } from '@pe/i18n-core';
import { FolderItem, PeFoldersRequestsErrorsEnum } from '@pe/shared/folders';
import { SnackbarService } from '@pe/snackbar';

import { PeFoldersUpdatePositionsInterface } from '../interfaces';
...
export class PeFoldersApiService {
  ...
  public getFolderItems(folderId: string, searchData?: any, rootFolderId: string = null): Observable<any> {
    let path: string;
    if (folderId === ID_OF_DEFAULT_FOLDER) {
      path = `${this.foldersDefaultThemePath}/documents`;
    } else {
      path = folderId === rootFolderId
  ...
  private getSearchParams(searchData: PeGridSearchDataInterface): HttpParams {
    ...
    let searchParams: HttpParams = new HttpParams()
      .set('sort[0][field]', searchDataCopy.orderBy ? searchDataCopy.orderBy : PeGridSortingOrderByEnum.CreationDate)
      .set('sort[0][direction]', searchDataCopy.direction ? searchDataCopy.direction : PeGridSortingDirectionEnum.Descending)
