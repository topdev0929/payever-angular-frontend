...
export class PeFolderComponent extends FolderTreeDraggable implements OnInit, OnChanges, OnDestroy {
  ...
  @Output() cdkDropListExited = new EventEmitter<CdkDragExit<any>>()
  @Output() cdkDropListDropped = new EventEmitter<FolderItemFlatNode>()
  ...
  @Input() set selectFolder(folder: FolderItem) {
    ...
    if (folder) {
      ...
      this.isSelectedRootItem = !this.isMobileWidth;
    }
  ...
  @Output() clickedFolder = new EventEmitter<FolderItem>();
  ...
  isEnablePaste = true;
  isMobileWidth = window.innerWidth <= 720;
  ...
  handleDrop(node: FolderItemFlatNode): void {
    this.cdkDropListDropped.emit(this.dragNode);
    super.handleDrop(node);
  }

  handleRootDrop(node: RootFolderItem){
    this.cdkDropListDropped.emit(this.dragNode);
    super.handleRootDrop(node);
  }
  ...
  nodeSelect(node: FolderItemFlatNode): void {
    ...
    this.clickedFolder.emit(this.selectedItem);
  }
  ...
  onCreateFolder(node: FolderItemFlatNode, name: string): void {
    ...
    const folderName: string = name.trim() || node.name;
    this.applyNode(node, folderName, () => {
      const folder = { ...newFolder, name: folderName };

      if (node.editing) {
        this.isCreating = true;
        this.folderDatabase.updateItem(folder, folderName);
  ...
  private duplicateFolder(node: FolderItem): void {
    ...
    const newFolder = this.folderDatabase.duplicate({
      ...node,
      _id: newId,
  ...
  private checkClientWidth(): void {
    this.isMobileWidth = window.innerWidth <= 720;
  }
