...
import { AppSetUpStatusEnum, MicroAppInterface, MicroRegistryService, PeDestroyService } from '@pe/common';
...
export class BusinessDashboardLayoutComponent extends AbstractDashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.showChatButton();
    this.platformService.backToDashboard$.pipe(takeUntil(this.destroy$)).subscribe(() => {
      this.showChatButton();
    });

    this.loaderService.appLoading$.pipe(takeUntil(this.destroy$)).subscribe(() => this.hideChatButton());
  }
