...
import { ExecuteCommands, PLACEHOLDER_ITEM_DATA_TOKEN } from '../text-editor.constants';
...
export class PeTextEditorPlaceholderComponent {
  ...
  constructor(
    @Inject(PLACEHOLDER_ITEM_DATA_TOKEN) private placeholderData: TextEditorPlaceholderItem[],
    private textEditorService: TextEditorService,
    private destroy$: PeDestroyService,
  ) {
    console.log(this.placeholderData);

    this.dataSource.data = this.placeholderData ?? PLACEHOLDER_DATA;
  ...
  public searchPlaceholders(search: string): void {
    ...
    this.dataSource.data = search ? this.search(PLACEHOLDER_DATA, search, addedMap).children : PLACEHOLDER_DATA;
