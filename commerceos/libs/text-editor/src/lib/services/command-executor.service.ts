...
export class CommandExecutorService {
  ...
  insertPlaceholder(placeholder: string, renderer: Renderer2, textarea: ElementRef<HTMLElement>): void {
    ...
    !this.savedRange && (this.savedRange = this.selection.getRangeAt(0));
  ...
  setListColor(range: Range, color: string, renderer: Renderer2): void {
    if (this.selection && range && hasList()) {
      ...
      if (!range.collapsed && isHtmlElement(range.commonAncestorContainer)
        ...
        for (const childNode of Array.from(range.commonAncestorContainer.childNodes)) {
          renderer.setStyle(childNode, 'color', color);
        }
      } else {
  ...
  setSelectionLinksColor(color: string, range: Range, selection: Selection, renderer: Renderer2): void {
    ...
    if (fragment && typeof fragment.querySelectorAll === 'function') {
      ...
      for (const link of Array.from(links)) {
        renderer.setStyle(link, 'color', color);
      }
    }
      ...
      if (parent &&
        ...
        if (fonts.length) {
          for (const font of Array.from(fonts)) {
            if (font.hasAttribute('size')) {
              ...
              renderer.setStyle(font, 'fontSize', `${fontSize}px`);
              renderer.setStyle(font, 'lineHeight', `${fontSize}px`);
            }
          }
        }
        ...
        if (lis.length) {
          for (const li of Array.from(lis)) {
            li.removeAttribute('size');
            renderer.setStyle(li, 'fontSize', `${fontSize}px`);
            renderer.setStyle(li, 'lineHeight', `${fontSize}px`);
          }
        }
        ...
        if (selectedFragment?.textContent.length) {
  ...
  setCaretToEnd(element: HTMLElement): void {
    if (this.selection) {
      ...
      while (node?.lastChild && node?.nodeType !== 3) {
        node = node.lastChild;
      }
  ...
  isInLastPosition(): boolean {
    return this.savedRange?.endContainer &&
  ...
  private cleanNodeChildsFontSize(node: DocumentFragment | HTMLElement, renderer: Renderer2): void {
    ...
    for (const nodeElement of Array.from(styledElements)) {
      renderer.setStyle(nodeElement, 'fontSize', '');
      renderer.setStyle(nodeElement, 'lineHeight', '');
    }
  }
