...
export class MicroRegistryService extends BaseMicroService {
  ...
  getMicroConfig(microCode?: string): MicroAppInterface | MicroAppInterface[] | undefined {
    ...
    return microCode ? registeredApps.find(app => app.code === microCode) : registeredApps;
  }

  loadBuild(micro: MicroAppInterface, forceReload = false): Observable<boolean> | null {
    if (forceReload) {
  ...
  private createBuildObservable(micro: MicroAppInterface, forceReload = false): Observable<boolean> {
