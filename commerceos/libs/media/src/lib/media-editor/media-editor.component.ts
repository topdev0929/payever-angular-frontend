...
export class PeMediaEditorComponent {
  ...
  public onFileDrop(event: DragEvent): void {
    ...
    this.addMedia(Array.from(event.dataTransfer.files));
  }

  public onFileLeave(event): void {
    event.preventDefault();
    event.stopPropagation();
    this.isDragging = false;
  }
  ...
  private countMedia(type: PeMediaFileTypeEnum, subtract: boolean): void {
    ...
    type === PeMediaFileTypeEnum.Image
      ? this.counterImageFiles += increment
      : this.counterVideoFiles += increment;
  }
  ...
  private async addMedia(files: File[]): Promise<void> {
    ...
    for (const file of files) {
      ...
      let fileType: PeMediaFileTypeEnum = null;
      if (isFileImage) {
        fileType = PeMediaFileTypeEnum.Image;
      }
      if (isFileVideo) {
        fileType = PeMediaFileTypeEnum.Video;
      }
