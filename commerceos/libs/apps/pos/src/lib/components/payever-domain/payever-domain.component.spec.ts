...
describe('PeSettingsPayeverDomainComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    apiSpy.updateAccessConfig.and.returnValue(of({ updated: true }) as any);
  ...
  it('should set done btn callback on construct', () => {
    ...
    api.updateAccessConfig.and.returnValue(of(null));

    config.doneBtnCallback();

    expect(api.updateAccessConfig).toHaveBeenCalledWith(appData._id, {
      internalDomain: '',
    });
    expect(appData.onSaved$.next).toHaveBeenCalledWith({ updatePosList: true });
    expect(overlay.close).toHaveBeenCalled();
  ...
  it('should validate domain', () => {
    ...
    expect(api.validateName).not.toHaveBeenCalled();
    ...
    api.validateName.and.returnValue(of({ message: undefined }));
    ...
    // w/ message
    api.validateName.and.returnValue(of({ message: 'test error' }));
