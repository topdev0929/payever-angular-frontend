...
export class PeSocialCalendarComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private sortingOrder: PeGridSortingInterface = {
    ...
    orderBy: PeGridSortingOrderByEnum.SortDate,
  };
  ...
  private readonly toggleSideNav$ = this.peSocialGridSideNavService.sideNavToggle$
    .pipe(
      tap(() => {
  ...
  constructor(
    ...
    private peSocialGridSideNavService: PeSocialGridSideNavService
  ) {
  ...
  private initRootFolder(): void {
    this.peFoldersApiService.getRootFolder().pipe(
      tap((rootFolder) => {
        rootFolder.name = this.translateService.translate('social-app.folders.all_posts');
  ...
  private paste(isDuplicated: boolean = false): void {
    ...
    switch (item.type) {
      ...
      case PeGridItemType.Item:
        this.peSocialApiService
          ...
          .pipe(
            map((post): Partial<PeSocialPostInterface> => {
              post.parentFolderId = this.selectedFolder._id;
              post.status = isDuplicated ? PeSocialPostStatusesEnum.Draft : post.status;
  ...
  private delete(): void {
    ...
    this.confirmScreenService.show(headings, true)
      .pipe(
        ...
        tap(() => {
          const intro = this.getIntroDeleteMessage(items.length, foldersIds.length);
  ...
  private getIntroDeleteMessage(selectedItemsCount: number, selectedFoldersCount: number): string {
    let text = '';

    if (selectedItemsCount > 0 && selectedFoldersCount > 0 ) {
      text = 'social-app.notify.all_selected_items';
    } else if (selectedItemsCount === 0 && selectedFoldersCount > 1) {
      text = 'social-app.notify.all_selected_folders';
    } else if (selectedItemsCount > 1 && selectedFoldersCount === 0) {
      text = 'social-app.notify.all_selected_posts';
    } else if (selectedItemsCount === 0 && selectedFoldersCount === 1) {
      text = 'social-app.notify.selected_folder';
    }  else if (selectedItemsCount === 1 && selectedFoldersCount === 0) {
      text = 'social-app.notify.selected_post';
    }
