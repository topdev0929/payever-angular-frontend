...
export class PeSocialPostEditorComponent implements OnInit, OnDestroy {
  ...
  public readonly availableIntegrations$ = this.peSocialEnvService.businessIntegrations$.pipe(
    map(integrations => {
      const availableIntegrations = integrations.filter(integration => integration.enabled && integration.channelId);
      ...
      return availableIntegrations.map(integration => {
        ...
        const isPosted = this.publishedChannelsIds.some(
          publishedChennelId => publishedChennelId === integration.channelId,
        );
        const integrationToList = integrationsList.find(
          enabledIntegration => enabledIntegration._id === integration.channelId,
        ) ?? {
        ...
        integrationToList.wasPosted = isPosted;
        isPosted &&
  ...
  private readonly contentChangesListener$ = merge(
    ...
    map(() => {
      ...
      const getWarning = (warningMessage: string) =>
        this.translateService.translate(`social-app.post_editor.channels.warnings.${warningMessage}`);

      return this.integrationsList$.value.map(integration => {
        ...
        if (media.length) {
          ...
          if (media[0].mediaMimeType === PeMediaFileTypeEnum.Image && imagesPerPost > maxImagesPerPost) {
            ...
            media.forEach((file: PeMediaInterface, index) => {
              ...
              // General media rules
              const maxFileSize =
                rules.maxFileSize && metadata.mediaSize ? metadata.mediaSize < rules.maxFileSize : true;
              const minAspectRatio =
                rules.aspectRatio.min && metadata.aspectRatio ? metadata.aspectRatio > rules.aspectRatio.min : true;
              const maxAspectRatio =
                rules.aspectRatio.max && metadata.aspectRatio ? metadata.aspectRatio < rules.aspectRatio.max : true;

              if (!maxFileSize) {
                warning = getWarning('max_file_size');
              } else if (!minAspectRatio || !maxAspectRatio) {
                warning = getWarning('aspect_ratio');
              }

              // Video rules
              const videoFile = mimeType === PeMediaFileTypeEnum.Video;
              const minDuration =
                videoFile && videoRules.duration.min && metadata.duration
                  ? metadata.duration > videoRules.duration.min
                  : true;
              const maxDuration =
                videoFile && videoRules.duration.max && metadata.duration
                  ? metadata.duration < videoRules.duration.max
                  : true;
              const maxWidth =
                videoFile && videoRules.maxResolution.width && metadata.resolution
                  ? metadata.resolution.width < videoRules.maxResolution.width
                  : true;
              const maxHeight =
                videoFile && videoRules.maxResolution.height && metadata.resolution
                  ? metadata.resolution.height < videoRules.maxResolution.height
                  : true;

              if (videoFile) {
                if (!minDuration) {
                  warning = getWarning('min_duration');
                } else if (!maxDuration) {
                  warning = getWarning('max_duration');
                } else if (!maxWidth) {
                  warning = getWarning('max_width');
                } else if (!maxHeight) {
                  warning = getWarning('max_height');
                }
              }
              ...
              const channelResolver =
                ...
                minAspectRatio &&
                maxAspectRatio &&
                (videoFile ? minDuration && maxDuration && maxWidth && maxHeight : true);

              if (mediaResolver) {
                mediaResolver = channelResolver;
              }
              if (!channelResolver) {
          ...
          const warningLabel = mediaResolver ? letterCount : warning;
  ...
  private readonly getFilteredData$ = this.setFilter$.pipe(
    filter(({ filter }) => {
  ...
  ngOnInit(): void {
    ...
    const getPost$ = postId ? this.getPost(postId) : of(null);
    const initPostForm$ = concat(this.availableIntegrations$.pipe(take(1)), getPost$).pipe(
      skip(1),
      tap(() => {
    ...
    merge(initPostForm$, this.contentChangesListener$, this.getFilteredData$)
  ...
  public filteredItems(control: AbstractControl) {
    return this.filterItems$.pipe(
      map(arrayToFilter =>
        arrayToFilter.filter(item => {
          const itemId = item?._id ?? item?.id;

          return !control.value.some(controlItem => controlItem?._id === itemId || controlItem?.id === itemId);
        }),
      ),
    );
  }
  ...
  private getProducts(filter: string = ''): Observable<any> {
    return this.peSocialApiService.getProducts(filter).pipe(this.errorHandler());
  }
  ...
  private updateContent(product?: {
    ...
    const postDescription = product
      ? [product.title, product.price + ' ' + product.currency, content.value].join('\n')
      : content.value.split('\n').splice(2).join('\n');
  ...
  private checkInsert(): boolean {
    const values = this.postForm.value;

    return (
      (values.type === PeSocialPostTypesEnum.Product && !!this.searchTextValue) ||
      values.media.length !== 0 ||
      values.products.length !== 0 ||
      values.setScheduleDate ||
      !!values.title ||
      !!values.toBePostedAt ||
      !!values.status ||
      !!values.content
  ...
  private closeEditor = () => {
    if (this.postForm.dirty && this.checkInsert()) {
  ...
  private getPost(postId: string): Observable<any> {
    return this.peSocialApiService.getSocialPost(postId).pipe(
      switchMap(postData =>
        postData.productId.length
        ...
        map((postData) => {
          ...
          const media = isMedia ? postData.media : [];
          const mediaFromPost = isAttachments ? postData.attachments : media;

        this.publishedChannelsIds = postData.channelSet;
        const availableChannels = postData.channelSet.filter(channel =>
          this.integrationsList$.value.some(integration => integration._id === channel),
        );
      ...
      switchMap(({ isAttachments, media }) => {
        const media$ = media.map((file): Observable<PeMediaInterface> => {
          return defer(async (): Promise<PeMediaInterface> => {
            const url = isAttachments ? file.previewUrl ?? file.url : file;
            const mimeType = this.peMediaService.getMediaType(url);

            return await this.peMediaService
              .getFileByUrl(url)
  ...
  openDatepicker(event, controlName: string): void {
    ...
    const dialogRef = this.dateTimePicker.open(event, {
      ...
      config: { headerTitle: 'Date', range: false, format: 'YYYY-MM-DD', maxDate: null, minDate: minDate },
    });
    ...
    dialogRef.afterClosed
      .pipe(
        ...
        tap(data => {
          ...
          const validDate = moment(date).format(dateFormat);
          this.postForm.get(controlName).setValue(validDate);
          this.postForm.get(controlName).markAsDirty();
          this.cdr.markForCheck();
        }),
      )
      .subscribe();
  ...
  public openTimepicker(event: MouseEvent, timeControl: AbstractControl): void {
    ...
    this.pebTimePickerService
      ...
      .afterClosed.pipe(
        take(1),
        tap((value) => {
          const time = value ?? timeToCorrect;
  ...
  public onSave(toSave: boolean): void {
    ...
    if (controls.type.value === PeSocialPostTypesEnum.Product && !controls.products.value?.length) {
      const productWarningText = this.translateService.translate('social-app.notify.invalid_product_post');
      this.showWarning(productWarningText);
    ...
    if (controls.setScheduleDate.value) {
      ...
      toBePostedAt = isDateValid && isTimeValid ? datetime.toISOString() : null;
    } else {
    ...
    if ((dirty || isPublishFromDraft) && valid) {
      if (!toSave) {
        this.showPublishConfirmDialog()
          .pipe(
            ...
            filter(Boolean),
            tap(() => {
              this.saveAndPublish(controls, postId, toSave, toBePostedAt);
            }),
            takeUntil(this.destroy$),
          )
          .subscribe();
      ...
      if (
        !toSave &&
        controls.channelSet.value.filter(channelId =>
          this.integrationsList$.value.some(channel => channel._id === channelId),
        ).length === 0
      ) {
        this.snackbarService.toggle(true, {
          content: 'Please ensure you select at least one channel for publishing.',
        });
      }
  ...
  private saveAndPublish(controls: { [key: string]: AbstractControl; },
    postId: any, toSave: boolean, toBePostedAt: string) {
    const scheduleStatus = controls.setScheduleDate.value
      ? PeSocialPostStatusesEnum.Schedule
      : PeSocialPostStatusesEnum.PostNow;
    const postData: PeSocialPostInterface = {
      channelSet: controls.channelSet.value.filter(channelId =>
        this.integrationsList$.value.some(channel => channel._id === channelId),
      ),
      content: controls.content.value.trimEnd().replace(/^\s+/, ''),
      media:
        controls.type.value === PeSocialPostTypesEnum.Product
          ? controls.products.value.reduce((images, product) => [...images, ...product.images], [])
          : controls.media.value.map(file => file.file ?? file.mediaUrl),
    ...
    of(postId)
      .pipe(
        switchMap(postId => {
          ...
          const createObs = postData.type === PeSocialPostTypesEnum.Media
            ? this.peSocialApiService.createPostOfMedia(postData)
            : this.peSocialApiService.createPostOfProduct(postData);

          return postId
            ? this.peSocialApiService.updatePostOfMedia(postId, postData)
            : createObs;
        }),
        map(post =>
          this.peOverlayData?.applicationScopeElasticId
            ? {
                ...post,
  ...
  private showPublishConfirmDialog() {
    const headings: Headings = {
      title: this.translateService.translate('Do you want to publish?'),
      subtitle: '',
      confirmBtnText: this.translateService.translate('Publish'),
      declineBtnText: this.translateService.translate('Cancel'),
    };

    return this.confirmScreenService.show(headings, true);
  }
