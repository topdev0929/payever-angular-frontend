...
export class PeSocialApiService {
  ...
  public updatePostOfMedia(postId: string, postTemplate: PeSocialPostInterface): Observable<any> {

    const formData = new FormData();
    postTemplate.media.forEach((file) => {
      formData.append('file', file);
    });
    const { ...rest } = postTemplate;
    const params = { post: JSON.stringify(rest) };

    return this.httpClient
      .patch(`${this.socialPath}/post/update-media-post/${postId}`, formData, { params })
      .pipe(
        catchError((error) => {
          this.errorHandler(PeSocialRequestsErrorsEnum.UpdatePost, error, true);
