...
import {
  PeSocialBusinessIntegrationInterface,
  PeSocialChannelSetInterface,
  PeSocialPostRuleInteface,
} from '../interfaces';
  ...
  public getChannelsSet(channelsSet: any[]): PeSocialChannelSetInterface[] {
    return channelsSet.map(({ channelSet, rules }) => ({
      ...
      maxlength: rules?.length
        ? rules.find((rule: PeSocialPostRuleInteface) =>
            rule.lettersCount !== null && rule.lettersCount !== 0)?.lettersCount
        : 2000,
