...
import { PeSocialApiService, PeSocialEnvService, PeSocialHeaderService, PeSocialGridSideNavService } from './services';
...
export class PeSocialComponent implements OnInit, OnDestroy {
  ...
  private addSidenavItem(): void {
    this.pePlatformHeaderService.assignSidenavItem({
      ...
      item: {
        ...
        onClick: () => {
          this.peGridSidenavService.toggleViewSidebar();
          const url = `/business/${this.envService.businessId}`;
          if (this.isMobile){
            this.router.navigate([`${url}/social/`]);
          }
        },
      },
    });
  ...
  public onFolderClick(folder: FolderItem): void {
    this.peSocialGridSideNavService.sideNavToggle = true;
  }
}
