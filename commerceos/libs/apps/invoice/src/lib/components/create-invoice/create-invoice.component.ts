...
export class PeCreateInvoiceComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.invoiceApiService.getCurrencyList().subscribe((res: any[]) => {
      this.currencies = res.slice().sort((pre, now) => pre.name > now.name ? 1 : -1).map((data) => {
    ...
    this.invoiceForm.valueChanges.pipe(takeUntil(this.destroy$)).subscribe((data) => {
  ...
  normalizeProductsData(products): any[] {
    return products.map((element) => {
      ...
      if (element.image) {
        ...
        image = this.mediaUrlPipe.transform(element.images[0], 'products');
      }
  ...
  normalizeContactsData(contacts): any[] {
    return contacts.map((element) => {
      ...
      if (element.image) {
        ...
        image = this.mediaUrlPipe.transform(element.images[0], 'contacts')
      }
