...
import { cloneDeep, forIn, isEqual, pick } from 'lodash-es';
import { BehaviorSubject, combineLatest, EMPTY, forkJoin, Observable, Subject } from 'rxjs';
import { catchError, distinctUntilChanged, finalize, map, startWith, take, takeUntil, tap } from 'rxjs/operators';
...
@Injectable()
export class PebInvoiceGridService implements OnDestroy {
  ...
  constructor(
    ...
    this.messageBus.listen('invoice.set.builder_edit').pipe(takeUntil(this.destroy$))
      .subscribe((item: any) => {
  ...
  createDataGridItem(invoice: any): PeDataGridItem {
    new Intl.NumberFormat(this.localeConstantsService.getLocaleId(), {}).format(invoice);
    const item = {
      id: invoice.id,
      name: invoice.name,
