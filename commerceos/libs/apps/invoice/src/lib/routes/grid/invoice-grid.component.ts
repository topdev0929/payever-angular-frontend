...
import { PeFilterChange, PeFilterConditions, PeFilterType, PeGridSearchFiltersInterface } from '@pe/grid/shared';
...
export class PeInvoiceGridComponent implements OnInit, OnDestroy {
    ...
    menuItems: [
      {
        title: this.translateService.translate('invoice-app.common.invoice.add_new_folder'), onClick: () => {
          this.openOverlay({ type: 'add_new', activeItem: {} }, null);
        },
      },
    ],
  };
  ...
  constructor(
    ...
    public actualPeInvoiceApi: PeInvoiceApi,
    private store: Store,
  ...
  selectSideNavMenu(menuItem: PeGridMenuItem) {
    ...
    if (menuItem.value === SideNavMenuActions.NewHeadline) {
      this.createHeadline();
    }
  }
  ...
  edit(item) {
    if (item.data) {
      this.openOverlay({ type: 'edit', activeItem: this.selectedFolder }, item);
    } else {
      this.actualPeInvoiceApi.getInvoiceById(item.id).subscribe((res) => {
        this.createEditInvoice(res);
  ...
  addFolder(node = null) {
    this.openOverlay({ type: 'add_new', activeItem: this.pebInvoiceGridService.activeNode }, node);
  }
