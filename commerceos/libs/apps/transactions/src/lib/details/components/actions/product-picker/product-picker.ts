...
@Component({
  selector: 'pe-action-product-picker',
  templateUrl: './product-picker.html',
  styleUrls: ['./product-picker.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ActionProductPickerComponent implements OnInit {
  ...
  private getRefundItem(id:string): FormGroup {
    return this.refundItems.controls.find((item: FormGroup) => item.controls.identifier.value === id) as FormGroup;
  }

  addSelectedProduct(option: RefundProductInterface): void {
    if (
      this.addedItems.includes(option) ||
      this.addedItems.find(item => item.id === option.id)
    ) {
      ...
      const refundItemsFormGroup: FormGroup[] = this.addedItems.map(
        (item: any) => {
          const formGroup = this.getRefundItem(item.id);

          return this.formBuilder.group({
            identifier: item.id,
            quantity: formGroup ? formGroup.controls.quantity.value : 0,
            name: item.name,
            price: item.price,
          });
        }
