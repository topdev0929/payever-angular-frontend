...
export class ActionsContainerService {
  ...
  private readonly hiddenWhenExecuting: {
    [key in ActionTypeEnum]?: ActionTypeEnum[]
  } = {
    [ActionTypeEnum.SendSigningLink]: [ActionTypeEnum.DownloadContract],
  }
  ...
  toggleHidden(runtimeActions: ActionTypeEnum[], uiActions: UIActionInterface[]): UIActionInterface[] {
    return uiActions.map((action) => {

      action.isHidden = runtimeActions.reduce((acc, item) => {
        return acc || this.hiddenWhenExecuting?.[item]?.includes(action.action);
      }, false);
