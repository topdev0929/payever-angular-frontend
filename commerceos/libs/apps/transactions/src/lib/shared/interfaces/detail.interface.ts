...
import { GuarantorTypeEnum, VerifyModeEnum } from '../enums';
...
export interface PaymentDetailsInterface extends PaymentSecurityInterface {
  ...
  is_verified?: boolean;
  signature_withdrawn?: boolean;

  // qr links
  customer_signing_link_qr?: string;
  guarantor_signing_link_qr?: string;
}
...
const DetailInternalValuesSample = {
  _ziniaId: '',
  _ziniaTransferReference: '',
  _isInvoice: false,
  _isSantander: false,
  _isSantanderDe: false,
  _isSantanderAt: false,
  _isSantanderDk: false,
  _isSantanderNo: false,
  _isSantanderNl: false,
  _isSantanderPosDeFactInvoice: false,
  _hideUpdateStatusAction: false,
  _showSantanderDeQr: false,
  _santanderApplicationNo: '',
  _applicationNo: '',
  _panId: '',
  _statusColor: '',
};

export const DetailInternalKeys = Object.keys(DetailInternalValuesSample);

export type DetailInternalValuesType = {
  [K in keyof typeof DetailInternalValuesSample]?: typeof DetailInternalValuesSample[K];
}


export interface DetailInterface extends DetailInternalValuesType {
...
export interface SectionsSubTitlesInterface {
  products: string;
  order: string;
  shipping: string;
  billing: string;
...
export interface RequestDataFields {
  documents: RequestDataDocument[];
}

export interface RequestDataDocument {
  documentType: string;
  file: File;
  filename: string;
}
