...
import { CustomHttpParamEncoder } from "../settings/strategy";
...
export class ApiService {
  ...
  postAction(orderId: string, action: ActionTypeEnum, payload: any): Observable<any> {
    const url: string = this.settingsService.postActionUrl(orderId, action);

    return this.http.post(url, payload);
  }

  processShippingOrder(order: ProcessShippingOrderInterface, shippingOrderId: string): Observable<void> {
    const endpointUrl: string = this.settingsService.postShippingOrder(shippingOrderId);
