...
import { EnvService, PeDataGridListOptions } from '@pe/common';
...
import { TransactionsStatusEnum } from '../shared';
...
export class TransactionsListService {
  ...
  getStatusColor(status: StatusType): string {
    let color: string;
    switch (status) {
      case TransactionsStatusEnum.STATUS_ACCEPTED:
      case TransactionsStatusEnum.STATUS_PAID:
        color = 'green';
        break;
      case TransactionsStatusEnum.STATUS_FAILED:
      case TransactionsStatusEnum.STATUS_CANCELLED:
      case TransactionsStatusEnum.STATUS_DECLINED:
        color = 'red';
        break;
      default:
        color = 'yellow';
