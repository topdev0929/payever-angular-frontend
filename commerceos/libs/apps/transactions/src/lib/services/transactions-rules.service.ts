  ...
  private folderTreeFlatten(tree: PeFolder[]): PeFolder[] {
    return tree.reduce((acc, folder) => {
      if (folder?.children?.length) {
        return acc.concat([
          folder,
          ...this.availableFolder(folder) ? this.folderTreeFlatten(folder.children) : [],
        ]);
      }

      return [
        ...acc,
        ...this.availableFolder(folder) ? [folder] : [],
      ];
    }, []);
  }

  private availableFolder(folder: PeFolder): boolean {
    return !folder.isHeadline && !folder.isProtected;
  }
}
