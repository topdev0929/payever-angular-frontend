...
import { FolderApply, FolderItem, FolderOutputEvent, FolderPosition, PeFoldersActionsEnum } from '@pe/shared/folders';
    ...
    switch (item.type) {
      ...
      case PeGridItemType.Item:
        this.peSubscriptionsApiService
          ...
          .pipe(
            switchMap((plan) => {
              const planToDuplicate: Partial<PeSubscriptionsPlanInterface> = omit(
                plan,
                [
                  '__v',
                  '_id',
            ...
            switchMap(([planToDuplicate, categories, products, subscribers, subscribersGroups]) => {
              ...
              return this.peSubscriptionsApiService.createPlan(planToDuplicate as PeSubscriptionsPlanInterface);
            }),
            tap((plan) => {
              const gridItem = this.peSubscriptionsGridService.plansToGridItemMapper([plan], this.canvas)[0];
