...
export class DataGridService implements OnDestroy {
  ...
  constructor(
    ...
    this.messageBus.listen('products.edit.menu').pipe(takeUntil(this.destroy$))
      .subscribe((item: any) => {
    ...
    this.messageBus.listen('products.delete.menu').pipe(takeUntil(this.destroy$))
      .subscribe((item: any) => {
        if (item?.data?.isFolder) {
  ...
  toggleOrderByField(field: ProductsOrderBy, direction?: Direction) {
    ...
    if (this.order.by === field) {
      updatedDirection = this.order.direction === Direction.ASC
        ? Direction.DESC
        : Direction.ASC
    } else {
      updatedDirection = Direction.DESC;
    }
    this.order = {
      by: field,
