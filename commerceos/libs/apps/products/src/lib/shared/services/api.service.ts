...
import { PeGridSortingInterface } from '@pe/grid';
import {
...
import { ProductConditions, ProductTypes } from '../enums/product.enum';
import {
...
export class ProductsApiService {
  public static readonly model: ProductModel = {
    id: null,
    images: [],
    brand: '',
    title: '',
  ...
  public static readonly collectionModel: CollectionModel = {
    _id: null,
    image: '',
    images: [],
    name: '',
  ...
  getProducts({
      ...
      order = { by: "", direction: Direction.DESC },
  ...
  getWidgetProducts(businessId: string, includeIds: string[], view = PeDataGridLayoutType.Grid): Observable<any> {
    const includeIdsFilter = includeIds ? `includeIds: [${includeIds.map(i => `"${i}"`)}]` : ''
  ...
  getProductTranslation(id:string, language: string): Observable<ApolloQueryResult<Product>> {
    return this.apollo
    .use(ApolloBaseName.products)
    .query({
  ...
  getProductCountrySetting(id:string, country: string): Observable<ApolloQueryResult<Product>> {
    return this.apollo
    .use(ApolloBaseName.products)
    .query({
  ...
  getProduct(id: string): Observable<ApolloQueryResult<Product>> {
    return this.apollo
      ...
      .pipe(
        switchMap((result: ApolloQueryResult<Product>) => {
          const prodData: any = (result || { data: {} }).data['product'] || {};
          if (prodData.businessUuid) {
            if (prodData.variants?.length) {
  ...
  removeStoreItem(ids: string[]): Observable<Responses.Empty> {
    return this.apollo.use(ApolloBaseName.products).mutate({
  ...
  getCategories(
    ...
    page: number,
    perPage: number
  ): Observable<ApolloQueryResult<{ getCategories: Category[] }>> {
    return this.apollo.use(ApolloBaseName.products).query({
  ...
  createCategory(businessUuid: string, title: string): Observable<FetchResult<{ createCategory: Category }>> {
    return this.apollo.use(ApolloBaseName.products).mutate({
  ...
  private getProductQLData(product: ProductModel, businessUuid: string): any {
    const productTmp = {
      ...
      channelSetCategories: product.channelSetCategories,
  ...
  private getShipping(shipping: ProductModelShipping): any {
    if (shipping) {
      return {
  ...
  getFolderDocuments(folderId: string, searchData: any) {
    ...
    return this.http.get<any>(path, { params: searchData ? this.getSearchParams(searchData) : null });
  }
