  ...
  implements OnInit, AfterViewInit
{
  ...
  public addPrice(price?): void {
    ...
    this.pricing.push(
      this.formBuilder.group({
        ...
        price: [price.price, [Validators.required, Validators.min(0), Validators.max(MAX_VALUE)]],
        salePrice: [price.salePrice, [Validators.min(0), Validators.max(MAX_VALUE)]],
      },{ validators: [
  ...
  getPriceErrorMessage(index: number): string {
    ...
    const form: FormGroup = pricing.controls[index] as FormGroup;

    if (!this.isSubmitted && !form.controls?.price?.errors?.max) {
      return null;
    }
    ...
    if (form.errors?.priceAndMainPriceNotEqual) {
      return this.translateService.translate('mainSection.form.errors.unequal_prices');
    }

    return null;
  }

  getSalePriceErrorMessage(index: number): string {
    const pricing = this.pricing;
    const form: FormGroup = pricing.controls[index] as FormGroup;
    if (!this.isSubmitted && !form.controls?.salePrice?.errors?.max) {
      return null;
    }

    if (form.controls?.salePrice?.errors?.required) {
      return this.translateService.translate('mainSection.form.errors.sale_price_required');
    }
    if (form.controls?.salePrice?.errors?.min) {
      return this.translateService.translate('mainSection.form.errors.price_no_negative');
    }
    if (form.controls?.salePrice?.errors?.max) {
      const field = this.translateService.translate('placeholders.salePrice');
      return this.translateService.translate('errors.max_number', { field });
    }
    if (form.errors?.salePriceGreater) {
