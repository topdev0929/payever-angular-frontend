...
import { dateMask, mapVariantFormDataToVariant, mapVariantToFormData } from '../../utils';
import {
  dateValidator,
...
export class VariantEditorComponent extends FormAbstractComponent<any> implements OnInit, OnDestroy {
  ...
  protected onUpdateFormData(formValues: VariantsSection): void {
    this.optionsFormArray.controls.forEach((control) => {
      ...
      const value = Array.isArray(controlValue) ? controlValue[0] : controlValue;
      control.get('extra')?.setValue({
        hex: this.variantStorageService.getColorHex(value),
      }, {
        onlySelf: true,
        emitEvent: false,
      });
    ...
    if (this.isVariantEdit) {
      ...
      const requests$ = variants.map((variant, i) => {
        variant.options = variant.options.map(option => ({
          ...option,
          ...option.type === VariantOptionSectionType.COLOR
            ? {
              extra: {
                hex: this.variantStorageService.getColorHex(option.value),
              },
            }
            : {},
        }));
  ...
  private initForm(variant: VariantsCreateFormData | VariantsEditFormData): void {
    ...
    const existingOptions = formFromStorage?.get('options');
  ...
  private createOptionsFormArray(options: Array<OptionsSection | OptionsSectionGrouped>): FormArray {
    const groups: FormGroup[] = (options || []).map(option =>
      this.formBuilder.group({
        ...
        extra: [{
          ...option.type === VariantOptionSectionType.COLOR
            ? { hex: this.variantStorageService.getColorHex(String(option.value)) }
            : {},
        }],
      }),
    );
  ...
  private createDefaultMultipleOptionValueFieldSet(): FormSchemeField {
    return {
      ...
      autocompleteChipsSettings: {
        ...
        autocompleteClassList: `pr-variants-autocomplete`,
