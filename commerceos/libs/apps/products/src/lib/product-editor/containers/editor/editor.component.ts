...
import { EmployeesNotificationService } from '@pe/shared/business';
...
export class EditorComponent implements OnInit {
  ...
  linkControlConfig: LinkControlInterface = {
    ...
    onClick: () => {
      this.sectionsService.save().pipe(filter((valid: boolean) => !!valid), takeUntil(this.destroy$)).subscribe(
        () => this.handleSave(),
  ...
  ngOnInit(): void {
    ...
    if (isEdit) {
      const model: ProductModel = get(this.route.snapshot, ['data', 'product', 'data', 'product'], null);
      this.model = {
        ...model,
        sale: {
          ...model.sale,
          saleStartDate: model.sale.saleStartDate &&
            moment(model.sale.saleStartDate).format('DD.MM.YYYY'),
          saleEndDate: model.sale.saleEndDate &&
            moment(model.sale.saleEndDate).format('DD.MM.YYYY'),
        },
        available: this.sectionsService.model.available ?? false,
        inventoryTrackingEnabled: model.inventoryTrackingEnabled ?? false,
        inventory: model.inventory ?? 0,
        lowInventory: model.lowInventory ?? 0,
        emailLowStock: model.emailLowStock ?? false,
  ...
  handleError(err: any): void {
    ...
    if (message === 'This value is already used' || message === 'Product with sku already exists') {
      ...
      let errorMsg: string;
      if (error?.statusCode === STATUS_FORBIDDEN) {
        errorMsg = this.employeesNotificationService.isEmployee(this.businessAccessOptions)
          ? this.translateService.translate('app.permission.insufficient.error')
  ...
  private filterSectionKeys(currentVariants: VariantsSection[], onSales: boolean, type?: ProductTypes)
    ...
    return Object.values(ProductEditorSections)
      .filter((section) => {
        if (section === ProductEditorSections.Shipping) {
