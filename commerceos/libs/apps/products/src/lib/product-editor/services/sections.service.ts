...
import { filter, finalize, map, skip, switchMap, take, tap } from 'rxjs/operators';
...
import { PeProductCustomerFieldInterface, PeProductCustomerInterface } from '../../collection-editor';
...
import { ChannelSetChangeEventInterface, ChannelSetInterface } from '../../shared/interfaces/channel-set.interface';
import { ProductModel, ProductModelShipping } from '../../shared/interfaces/product.interface';
...
export class SectionsService {
  ...
  channelSets$: Subject<ChannelSetChangeEventInterface> = new Subject<ChannelSetChangeEventInterface>();
  channelSetsCategories$: Subject<ChannelSetCategoriesInterface[]> = new Subject<ChannelSetCategoriesInterface[]>();
  ...
  get productCondition(): ProductConditions {
    return get(this.model, 'condition', ProductConditions.New);
  }
  ...
  get mainSection(): MainSection {
    return {
      ...
      brand: get(this.model, 'brand', ProductsApiService.model.brand),
    ...
    return {
      categories:
        get(
          this.model,
          'categories',
          ProductsApiService.model.categories,
        ) || ProductsApiService.model.categories,
      ...
      channelSetCategories:
        get(
          this.model,
          'channelSetCategories',
          ProductsApiService.model.channelSetCategories,
        ) || ProductsApiService.model.channelSetCategories,
  ...
  get recommendationsSection(): RecommendationsSection {
    return {
      allowRecommendations: !!get(this.model, 'recommendations.tag', false),
  ...
  get visibilitySection(): { active: boolean } {
    ...
    if (channelSetId) {
      ...
      active = active ?? true; // not migrated product
    } else {
  ...
  get shippingSection(): ProductModelShipping {
    return {
  ...
  onChangeMainSection(mainSection: MainSection): void {
    ...
    this.model.brand = mainSection.brand;
    this.model.price = mainSection.price;
    this.model.condition = mainSection.condition;
    this.model.sale = {
  ...
  getCustomerIds(isCustomer: boolean, data: PricingSection): string[] {
    if (isCustomer && data.customerEligibilitySpecificCustomers?.length > 0) {
      return data.customerEligibilitySpecificCustomers.map(customer => customer?.id);
    }
    if (data.customerEligibilityCustomerGroups?.length > 0) {
      return data.customerEligibilityCustomerGroups.map(group => group?.id);
    }
  ...
  onChangeCategorySection(
    categorySection: { categories: NotFullCategory[], channelSetCategories?: ChannelSetCategoriesInterface[]},
  ): void {
    this.model.categories = categorySection.categories;
    this.model.channelSetCategories = categorySection.channelSetCategories;
    this.channelSetsCategories$.next(categorySection.channelSetCategories);
  }
  ...
  onChangeChannelsSection(marketplacesSection: ChannelsSection, isChecked: boolean): void {
    ...
    this.channelSets$.next({ channel: marketplacesSection, isChecked });
  }
  ...
  onChangeShippingSection(shipping: ShippingSection): void {
    this.model.shipping = {
      weight: shipping.weightSize,
      length: shipping.lengthSize,
      width: shipping.widthSize,
      height: shipping.heightSize,
    };
  }
        ...
        .createProduct(
          {
            ...
            channelSetCategories: this.model.channelSetCategories,
  ...
  onFindError(hasErrors: boolean, section: ProductEditorSections): void {
    ...
    } else if (this.hasErrors(section)) {
