...
import { BehaviorSubject, forkJoin, fromEvent, merge, Observable, of, Subject } from "rxjs";
import {
  debounceTime,
  filter,
  skip,
...
export class PeAffiliatesProgramsComponent implements OnInit, OnDestroy {
  ...
  private openEditor(gridItem?: PeGridItem, actionData?: PeFolderEditorActionDataInterface): void {
    ...
    if (isFolder) {
      formTitle = gridItem?.title ?? this.translateService.translate('folders.folder_editor.create_folder');
    } else if (itemData) {
      formTitle = this.translateService.translate('affiliates-app.program_editor.edit_program')
    } else {
      formTitle = this.translateService.translate('affiliates-app.program_editor.create_program');
    }
        ...
        this.peAffiliatesApiService
          ...
          .pipe(
            switchMap((program: PeAffiliatesProgramInterface) => {
              const programToDuplicate: Partial<PeAffiliatesProgramInterface> = omit(
                program,
                [
                  '__v',
                  '_id',
              ...
              if (program.appliesTo === PeAffiliatesProgramAppliesToEnum.SpecificCategories) {
                products$ = this.peAffiliatesApiService.getCategories();
              } else if (program.appliesTo === PeAffiliatesProgramAppliesToEnum.SpecificProducts) {
                products$ = this.peAffiliatesApiService.getProducts();
              } else {
                products$ = of(null);
              }
            ...
            switchMap(([programToDuplicate, productsForApplies]) => {
              ...
              return this.peAffiliatesApiService.createProgram(programToDuplicate as PeAffiliatesProgramInterface);
            }),
            tap((program) => {
              const gridItem = this.peAffiliatesGridService.programsToGridItemMapper([program], this.canvas)[0];
  ...
  private delete(): void {
    ...
    this.peAffiliatesGridService.confirmation$
      .pipe(
        ...
        tap(() => {
          let introKey: string;
          if (items.length && foldersIds.length) {
            introKey = 'affiliates-app.notify.all_selected_items';
          } else if (items.length) {
            introKey = 'affiliates-app.notify.all_selected_programs';
          } else {
            introKey = 'affiliates-app.notify.all_selected_folders';
          }
          const intro = this.translateService.translate(introKey);
