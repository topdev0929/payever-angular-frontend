...
export class PebShippingSettingsService {
  ...
  getProfileTitle(profile) {
    ...
    if (!profile?.zones?.[0]?.countryCodes) {
      countryText = '';
    } else if (profile?.zones[0]?.countryCodes.length > 1) {
      countryText = profile?.zones[0]?.countryCodes.length + ' countries';
    } else {
      countryText = this.countriesPipe.transform(profile?.zones[0]?.countryCodes);
    }

    return `${this.translateService.translate('shipping-app.labels.shipping_to')} ${countryText} `;
  }

  getZonePrice(zone: any, currency: string): string {
    if (zone?.rates?.length !== 1) {
      return `${this.translateService.translate('shipping-app.labels.starting_from')}
    ...
    return zone.rates[0].ratePrice === 0
      ? zone.rates[0].name
