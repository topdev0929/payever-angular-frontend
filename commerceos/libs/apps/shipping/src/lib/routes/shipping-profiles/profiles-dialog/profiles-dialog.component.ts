...
import {
  catchError, debounceTime, filter,
  skip, switchMap, take, takeUntil, tap, startWith, pluck, map,
} from 'rxjs/operators';
...
export class PebShippingProfileFormComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  normalizeProductsData(products): FormProductItem[] {
    return products.map((element) => {
      ...
      if (element.image) {
        ...
        image = this.mediaUrlPipe.transform(element.images[0], MediaContainerType.Products, 'grid-thumbnail' as any);
      }
  ...
  onEditZone = (e) => {
    this.shippingZoneService.getShippingZoneById(this.profilesForm.get('zonesData').value[e]).pipe(
      take(1),
      switchMap((response: any) => {
        ...
        return this.zoneRef.afterClosed.pipe(
          take(1),
          filter(data => !!data),
          switchMap(data => this.shippingZoneService.editShippingZone(data.id, data.data).pipe(
            tap((zone: any) => {
