...
export class AclsService implements OnDestroy {
  ...
  get appsTitle(): Map<string, string> {
    return this.currentAppsTitle;
  }
  ...
  init(options: AclsServiceInitOptionsInterface) {
    ...
    return this.options.apps.reduce((acc: AppInterface[], app: AppInterface) => {
      ...
      this.currentAppsTitle.set(app.code, this.translateService.translate(app.appName));
  ...
  private onToggleChange(appControl: AbstractControl, appDataCurrent: AppAclFormInterface) {
    ...
    const readingAccessOptions: AppAclFormOptionsEnum[] = [AppAclFormOptionsEnum.reading];
    const modifyAccessOptions: AppAclFormOptionsEnum[] = [AppAclFormOptionsEnum.creating, AppAclFormOptionsEnum.deleting, AppAclFormOptionsEnum.editing];

    const isAllOptionsSetToTrue = (modifyAccessOptions.length > 0 ? modifyAccessOptions : options).every((option: string) => appDataCurrent[option]);
    ...
    const modifyAccessOptionsWasEnabled = modifyAccessOptions.some((option: string) => appDataCurrent[option]);
    ...
    } else if (modifyAccessOptionsWasEnabled) {
      this.setValueForOptions(appControl, readingAccessOptions, true);
    }
