...
import { needApproveQueryParam } from '../../../../misc/constants';
...
export class PebGridDataConverterService {
  ...
  convertEmployeeToGridItem(employee: BusinessEmployeeInterface): EmployeesGridItemInterface {
    ...
    const data: EmployeesGridItemDataInterface = {
      ...
      [needApproveQueryParam]: employeePosition.status === EmployeeStatusEnum.needApprove
        ? needApproveQueryParam
        : null,
    ...
    const title = employee.fullName?.trim().length ? employee.fullName : employee.email;
