...
import { IErrorAPIResponse } from '@pe/api';
...
import {
  ...
  UserReportSettingsInterface,
  BusinessEmployeeInterface,
} from '../misc/interfaces';
import {
  BusinessDetailsInterface,
  BusinessTrustedDomainsInterface,
  TransactionRetentionSetting,
} from '../misc/interfaces/business-details.interface';
import { IGroupItemInterface, IGroupsInterface } from '../routes/employees/interfaces/employee-group.interface';
...
export class ApiService {
  ...
  getBusinessDetails(businessId: string): Observable<BusinessDetailsInterface> {
    const config = this.env.backend;

    return this.http.get<BusinessDetailsInterface>(`${config.users}/api/business/${businessId}/detail`);
  }

  getBusinessTaxes(businessId: string): Observable<BusinessTaxes> {
    const config = this.env.backend;

    return this.http.get<BusinessTaxes>(`${config.users}/api/business/${businessId}/taxes`);
  }

  getTransactionsRetentionSetting(businessId: string) {
    const config = this.env.backend;

    return this.http.get<TransactionRetentionSetting>(`${config.transactions}/api/business/${businessId}/transaction-retention-setting`);
  }

  patchTransactionsRetentionSetting(businessId: string, data: TransactionRetentionSetting) {
    const config = this.env.backend;

    return this.http.patch<TransactionRetentionSetting>(`${config.transactions}/api/business/${businessId}/transaction-retention-setting`, data);
  }

  getBusinessTrustedDomains(businessId: string): Observable<BusinessTrustedDomainsInterface[]> {
    return this.http.get<BusinessTrustedDomainsInterface[]>(`${this.env.backend.users}/api/trusted-domain/${businessId}`);
  }

  updateBusinessTrustedDomains(businessId: string, domain: string): Observable<BusinessTrustedDomainsInterface> {
    const config = this.env.backend;

    return this.http.post<BusinessTrustedDomainsInterface>(`${config.users}/api/trusted-domain/${businessId}`, {
      domain,
      businessId,
    });
  }

  deleteBusinessTrustedDomains(businessId: string, domain: string): Observable<BusinessTrustedDomainsInterface> {
    const config = this.env.backend;

    return this.http.request<BusinessTrustedDomainsInterface>('delete', `${config.users}/api/trusted-domain/${businessId}`, {
      body: { domain },
    });
  }
  ...
  createBusinessEmployeeGroup(
    ...
    return this.http.post<IGroupItemInterface>(url, group);
  }
  ...
  updateBusinessEmployeeGroup(
    ...
    return this.http.patch<IGroupItemInterface>(url, group);
  }
  ...
  deleteEmployeeFromGroup(
    ...
    return this.http.post<BusinessEmployeeInterface>(url, { employees });
  }

  getBusinessEmployeeList(businessId: string, limit: number, page: number): Observable<PaginationResponseInterface<BusinessEmployeeInterface>> {
    const config = this.env.backend;
  ...
  importFromFile(
    ...
    return this.http.post<any>(`${this.env.backend.users}/api/employees/${businessId}/bulk-create`, formData);
  }
  ...
  getSecurityQuestions(): Observable<string[]> {
    ...
    return this.http.get<any>(`${config.auth}/api/security-question`);
  }

  setSecurityQuestions(data: SecurityQuestionInterface): Observable<void> {
    const config = this.env.backend;

    return this.http.post<any>(`${config.auth}/api/security-question/define`, data);
  }
  ...
  getEmployeesForApprove(businessId: string) {
    return this.http.get(`${this.env.backend.auth}/api/employees/business/${businessId}/need-approval`);
  }

  addEmployeeToBusiness(userId: string, businessId: string): Observable<void> {
    const url = `${this.env.backend.auth}/api/employees/business/${businessId}/approve/employee/${userId}`;
