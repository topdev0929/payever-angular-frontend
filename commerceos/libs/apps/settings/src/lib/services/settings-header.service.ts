...
import { BusinessState, LoadBusinessData } from '@pe/user';
...
export class PeSettingsHeaderService extends BaseHeaderService {
  ...
  initialize() {
    ...
    this.messageBus.listen('settings.change.logo').pipe(takeUntil(this.destroy$)).subscribe((url: string) => {
    ...
    this.messageBus.listen('settings.close.app').pipe(takeUntil(this.destroy$)).subscribe((res) => {
      this.closeSettings();
    });
