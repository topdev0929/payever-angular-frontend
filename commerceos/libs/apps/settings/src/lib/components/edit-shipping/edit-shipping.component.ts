...
export class EditShippingComponent implements OnInit {
  ...
  onCheckValidity() {
    this.address.controls.forEach((rate) => {
        rate.get('city').setValidators([Validators.required, Validators.pattern(/[\S]/g)]);
        ...
        rate.get('street').setValidators([Validators.required, Validators.pattern(/[\S]/g)]);
        rate.get('street').updateValueAndValidity();

        rate.get('zipCode').setValidators([Validators.required, Validators.pattern(/[\S]/g)]);
