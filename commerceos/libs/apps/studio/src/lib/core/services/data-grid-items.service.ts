...
import { FolderApply, FolderItem, FolderOutputEvent } from '@pe/shared/folders';
import { StudioAppState } from '@pe/shared/studio';
...
import { PeCreateUserMedia, PeStudioAlbum, PeStudioCategory, PeStudioMedia } from '../interfaces';
...
export class DataGridItemsService {
  ...
  makeGridItem(media, mediaView) {
    return {
      id: media._id,
      title: media.name,
      image: this.mediaTypeGridPlaceholder(media),
      ...
      action: {
        label: this.translateService.translate('studio-app.grid.action_open'),
        more: true,
      },
      columns: [
        {
  ...
  private mediaTypeGridPlaceholder (media: PeStudioMedia): string {
    switch (media.mediaType) {
      case MediaType.Text:
        return 'assets/icons/text-place.svg';
      case MediaType.Model:
      case MediaType.Script:
        return 'assets/icons/script-place.svg';
      case MediaType.Video:
        return media.url + '_preview';
      default:
        return media.url ?? 'assets/icons/image-place.svg';
    }
  }

  openMediaPreview(id: string, mediaView: string, mediaType: MediaType) {
    ...
    singleMedia = { ...singleMedia, businessId: this.envService.businessId, theme, mediaView, mediaType };
  ...
  private editTextItem(gridItem: PeGridItem): Observable<HttpEvent<PeStudioMedia>> {
    return this.uploadTextService.openTextEditor({
      ...
      description: gridItem.data.description,
    }).pipe(
  ...
  private editMediaItem(gridItem): Observable<any> {
    return this.studioApiService.getUserSubscriptionMediaById(gridItem.id).pipe(
      switchMap((image) => {
  ...
  deleteMedia(gridItems: PeGridItem[]) {
    ...
    const headings: Headings = {
      ...
      subtitle: this.translateService.translate('studio-app.grid.delete_message') + ' ' + wordsMultiple,
      confirmBtnText: this.translateService.translate('studio-app.grid.delete'),
      declineBtnText: this.translateService.translate('studio-app.grid.cancel'),
    };
