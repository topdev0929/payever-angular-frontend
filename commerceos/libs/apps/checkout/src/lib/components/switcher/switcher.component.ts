...
export class SwitcherComponent extends BaseSwitcherComponent {
  ...
  openEditCheckoutModalLoading = false
  ...
  resetCheckout() {
    ...
    this.currentCheckout$ = this.storageService.getCheckoutById(this.checkoutUuid)
      .pipe(takeUntil(this.destroy$), filter(d => !!d));
  ...
  openEditCheckoutModal(checkout: CheckoutInterface) {
    ...
    this.openEditCheckoutModalLoading = true;
    this.currentCheckout$.pipe(filter(d => !!d), take(1), tap((d) => {
      this.isCreateMode = false;
      this.isModalOpen = true;
      this.openEditCheckoutModalLoading = false;
    })).subscribe();
  }
