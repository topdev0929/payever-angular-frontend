...
export class CreateCheckoutComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.currentCheckout$.pipe(takeUntil(this.destroy$)).subscribe();
  ...
  setModalOptions() {
    ...
    this.onSuccessSubject$.asObservable()
      .pipe(filter(val => !!val), debounceTime(300), takeUntil(this.destroy$))
      .subscribe(checkout => this.onSuccess(checkout));

    this.onDeletedSubject$.asObservable()
      .pipe(filter(val => !!val), takeUntil(this.destroy$))
      .subscribe(() => {
