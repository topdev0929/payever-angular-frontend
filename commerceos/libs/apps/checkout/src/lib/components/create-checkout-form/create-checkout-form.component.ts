...
const ERRORS: { [key: string]: string } = {
  required: 'create_checkout.validation.errors.required',
  maxlength: 'create_checkout.validation.errors.longName',
  forbiddenName: 'create_checkout.validation.errors.unavailableName',
  oldName: 'create_checkout.validation.errors.oldName',
};
...
export class CreateCheckoutFormComponent extends FormAbstractComponent<CheckoutFormInterface> implements OnInit {
      ...
      tap((val) => {
        if (val) {
          ...
          this.storageService.deleteCheckout(this.checkoutUuid)
            ...
            }, err => err && this.showError(err.message));
        }
      }),
      takeUntil(this.destroyed$)
    ).subscribe();
  ...
  parseFormErrors(): string {
    if (this.form.valid) {
      return '';
    }

    const errorKeys = Object.keys(this.form.get('name').errors);

    return this.translateService.translate(ERRORS?.[errorKeys[0]]);
  }
