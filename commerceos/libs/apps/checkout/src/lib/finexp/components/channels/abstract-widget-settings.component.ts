...
import {
  ...
  PayeverPaymentWidgetLoader,
  ExpandedCustomWidgetConfigInterface,
} from '@pe/finexp';
...
export abstract class AbstractWidgetSettingsComponent
  ...
  protected readonly minWidthLimit: number = 200;
  protected readonly minHeightLimit: number = 30;
  protected readonly defaultHeight: number = 40;

  protected readonly minWidthLimitByPayment: {
    [key in PaymentMethodEnum]?: number;
  } = {
    [PaymentMethodEnum.GOOGLE_PAY]: 100,
    [PaymentMethodEnum.APPLE_PAY]: 100,
  };

  protected readonly heightEnabledForPayments: PaymentMethodEnum[] = [
    PaymentMethodEnum.APPLE_PAY,
    PaymentMethodEnum.GOOGLE_PAY,
  ];

  protected readonly themeEnabledForPayments: PaymentMethodEnum[] = [
    PaymentMethodEnum.APPLE_PAY,
    PaymentMethodEnum.GOOGLE_PAY,
  ];
  ...
  defaultWidgetConfig: ExpandedCustomWidgetConfigInterface = defaultCustomWidgetConfig;
  protected selectedPaymentOption$ = this.widgetLoader.selectedPaymentOption$;

  widgetConfig: ExpandedCustomWidgetConfigInterface;
  ...
  ngOnInit(): void {
    ...
    this.storageService.getCheckoutByIdOnce(this.checkoutUuid).pipe(
      ...
      (response: ExpandedCustomWidgetConfigInterface) => {
        if (response) {
  ...
  initWidgetConfig(config: ExpandedCustomWidgetConfigInterface): void {
  ...
  setWidgetAmount(payments: WidgetConfigPaymentInterface[], paymentMethod: PaymentMethodEnum = null): void {
    const paymentIndex = paymentMethod
      ? payments?.findIndex(item => item.paymentMethod === paymentMethod)
      : payments.length - 1;

    this.widgetAmount = payments.length && payments[paymentIndex]
      ? (payments[paymentIndex].amountLimits.min + payments[paymentIndex].amountLimits.max) / 2
      : 2500;
  ...
  private addWidgetToPage(config: ExpandedCustomWidgetConfigInterface): void {
