...
import { take, catchError, flatMap, map, filter, tap, switchMap } from 'rxjs/operators';
...
export class StorageService implements FinexpStorageAbstractService { // TODO Cleanup
  ...
  setDefaultCheckout(checkoutId: string): Observable<void> {
    ...
    return this.apiService.setDefaultCheckout(businessUuid, checkoutId).pipe(flatMap((data) => {
  ...
  deleteCheckout(checkoutId: string): Observable<void> {
    ...
    return this.apiService.deleteCheckout(businessUuid, checkoutId).pipe(flatMap((data) => {
      return combineLatest(this.getCheckoutsOnce(true),
      this.getIntegrationsInfoOnce(true), this.getChannelSetsOnce(true)).pipe(map(() => {
        return null;
      }));
  ...
  getChannelSets(reset: boolean = false): Observable<CheckoutChannelSetInterface[]> {
    if (!this.channelSetsProcessed || this.businessUuid !== this.channelSetsBusinessUuid || reset) {
      ...
      return this.apiService.getChannelSets(this.businessUuid).pipe(
        tap(data => this.channelSetsSubject.next(data)),
        switchMap(() => this.channelSetsSubject.asObservable()),
        this.catchErrorPipe()
      );
    }
  ...
  getChannelSetsForCheckout(checkoutId: string, reset: boolean = false): Observable<CheckoutChannelSetInterface[]> {
    ...
    return this.getChannelSets(reset).pipe(
      map(d => d ? d.filter(item => item.checkout === checkoutId) : null),
    );
  }
  ...
  Observable<CheckoutChannelSetInterface[]> {
    return this.getChannelSetsForCheckout(checkoutId, reset).pipe(
      map(d => d
        ? d.filter(item => item.type === channelType)
        : null,
      ),
    );
  }
  ...
  attachChannelSetToCheckout(channelSetId: string, checkoutId: string): Observable<void> {
    return this.apiService.attachChannelSetToCheckout(
      ...
      return this.getChannelSets(true).pipe(take(1), map(() => {
        return null;
      }));
    }));
  }
    ...
    return catchError((error: any) => {
      if (error) {
        const errorMessage = error.status === 403
          ? this.translateService.translate('errors.forbidden')
          : error.message;
  ...
  getCheckoutNotificationSettingsOnce(): Observable<BusinessNotificationSettingsInterface[]> {
    return this.apiService.getCheckoutSettings(this.businessUuid).pipe(this.catchErrorPipe());
  }
  ...
  patchInstalledCheckoutConnectionsSortOrder(checkoutId: string, data: any): Observable<any> {
    return this.apiService.patchInstalledCheckoutConnectionsSortOrder(this.businessUuid, checkoutId, data);
  }
}
