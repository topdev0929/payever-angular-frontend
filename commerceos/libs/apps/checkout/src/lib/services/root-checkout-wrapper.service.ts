...
export class RootCheckoutWrapperService {
  ...
  updateSettings$: Observable<TimestampEvent> = null;
  ...
  private updateSettingsSubject = new BehaviorSubject<TimestampEvent>(null);
  ...
  onSettingsUpdated(): void {
    this.updateSettingsSubject.next(new TimestampEvent());
  }
  ...
  preparePrefilled(callback: PrefilledLinkCallback, paymentLinkId: string = null): void {
    if (!this.isSaving) {
      this.isSaving = true;
      this.callback = callback;
      this.checkoutLoadingSubject.next(true);
      this.saveFlowToStorageSubject.next(new TimestampEvent(paymentLinkId));
    }
  }
