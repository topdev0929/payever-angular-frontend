...
import { PeGridView, PeListImagesService } from '@pe/grid';
...
export class ConnectListService {
  ...
  getPaginationResult() {
    ...
    if (window.innerWidth <= 1000) {
      ...
      perPage = 16;
    } else if (window.innerWidth > 1700 && window.innerWidth <= 1920) {
      perPage = 25;
    } else {
      perPage = 30;
    }
  ...
  filterItems(translate, data) {
    const integrations = data.map((item) => {
      ...
      title = title === item?.displayOptions?.title || !title ? translate(item?.displayOptions?.title) : title;
