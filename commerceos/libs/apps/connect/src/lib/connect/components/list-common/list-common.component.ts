...
export class ListCommonComponent extends BaseGridClassDirective implements OnInit, OnDestroy {
  ...
  itemContextMenu(isInstalled: boolean, isTable? :boolean) {
    if (isInstalled) {
      return {
        items: [
          {
      ...
      return {
        items: [
          {
            label: this.translateService.translate('connect-app.actions.install'),
            value: PeContextMenu.Install,
          },
        ],
      };
    }
  ...
  private initTreeFilters(): Observable<any> {
    ...
    return this.integrationsStateService.getMyAppsFolders().pipe(
      ...
      tap((folders: FolderItem[]) => {
        ...
        this.categoriesTreeData = folders
          .filter(folderRaw => folderRaw.name !== 'integrations.categories.accountings') // disabled, since we do not have any apps in it.
          .map((folderRaw) => {
  ...
  handleOpen(integration: IntegrationInfoWithStatusInterface, isLoading: BehaviorSubject<boolean>): void {
    ...
    if (integration.status?.installed && modalComponents[integration.category]?.[integration.name]) {
      this.initModal(
    ...
    switch (menuItem?.value) {
      case PeContextMenu.Uninstall:
        ...
        this.paymentsStateService.installIntegrationAndGoToDone(false, gridItem.cardItem).pipe(
          ...
          finalize(() =>  gridItem.isLoading$?.next(false)),
        ).subscribe();
        break;
      case PeContextMenu.Install:
        this.onInstallCallback(gridItem.id, gridItem.isLoading$);
        break;
    }
  }
