...
export class ShopifyMainComponent extends ApiKeysBaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.apiKeys$ = this.stateService.getPluginApiKeys(this.integrationName).pipe(takeUntil(this.destroy$));
    ...
    ).pipe(filter(a => !!a), takeUntil(this.destroy$));
    ...
    this.pluginInfo$.pipe(filter(d => !!d), takeUntil(this.destroy$)).subscribe((info) => {
    ...
    this.isAddingKey$.pipe(takeUntil(this.destroy$)).subscribe((isAddingKey) => {
      this.integrationInfo$.pipe(filter(d => !!d), takeUntil(this.destroy$)).subscribe((info) => {});
    });
  }
