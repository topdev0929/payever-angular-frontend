...
export class InfoBoxGeneratorFormComponent extends FormAbstractComponent<DynamicInfoBoxGeneratorFormData> implements OnInit {
  ...
  get fieldset(): FormSchemeField[] {
    return this.formScheme?.fieldsets.fieldset;
  }
  ...
  @Output() changed: Subject<DynamicInfoBoxGeneratorFormData> = new Subject();
  @Output() submitted: Subject<DynamicInfoBoxGeneratorFormData> = new Subject();
    ...
    if (this.fieldset) {
      this.fieldset.forEach(({ name, fieldSettings }) => {
        if (fieldSettings) {
          ...
          if (settings?.classList?.includes('disabled')) {
  ...
  onUpdateFormData(): void {
    if (!isEqual(this.lastFormValue, this.form.value)) {
      ...
      if (!firstRun) {
        ...
        this.changed.next(this.form.value as DynamicInfoBoxGeneratorFormData);
      }
    }
  }
  ...
  private submitForm(): void {
    this.submitted.next(this.form.value as DynamicInfoBoxGeneratorFormData);
  }
