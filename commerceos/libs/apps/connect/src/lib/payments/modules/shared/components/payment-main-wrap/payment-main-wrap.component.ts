...
export class PaymentMainWrapComponent extends BasePaymentAccordionComponent implements OnInit, AfterViewChecked {
  ...
  constructor(injector: Injector) {
    ...
    // For panels:
    this.paymentData$ = this.payment$.pipe(takeUntil(this.destroy$), filter(d => !!d),
  ...
  ngOnInit(): void {
    ...
    this.isAddingConnVariant$.pipe(takeUntil(this.destroy$)).subscribe((isAdding) => {
      if (!isAdding) {
  ...
  deleteConnectionVariant(event: Event, index: number): boolean {
    return this.disconnectConnectionVariant(event, index);
  }
