...
import { BehaviorSubject, EMPTY, forkJoin, fromEvent, merge, Observable, of, Subject } from 'rxjs';
import { catchError, filter, map, skip, startWith, switchMap, take, takeUntil, tap } from 'rxjs/operators';
...
import { AddContacts, ContactsAppState, SetStatuses } from '@pe/shared/contacts';
...
import { ContactComponent, ContactsWidgetComponent, GroupModalComponent, StatusCellComponent } from '../../components';
import { Contact, ContactMainInfo, StatusField } from '../../interfaces';
import {
  ContactsGQLService,
  ContactsListService,
  ContactsRuleService,
...
export class PeContactsListComponent implements OnDestroy, OnInit {
  ...
  private onUpdateFolder$ = new Subject<FolderItem>();
  ...
  private statusCellComponentFactory = this.componentFactoryResolver.resolveComponentFactory(StatusCellComponent);
  public cdkDropListExitedCopyElementId$ = new BehaviorSubject<string>(null);
  ...
  public gridLayout: PeGridView = PeGridView.List;
  ...
  public readonly tableDisplayedColumns$ = this.listColumnsService.displayColumnsList$.pipe(
    map(columns =>
      columns.map(item =>
        item.name === 'statusName'
          ? {
              ...item,
              cellComponentFactory: this.statusCellComponentFactory,
            }
          : item,
      ),
    ),
  ...
  public toolbar$: Observable<{
    filterConfig: PeFilterKeyInterface[];
    customMenus: PeCustomMenuInterface[];
    optionsMenu: PeGridMenu;
    sortMenu: PeGridMenu;
  }> = this.store.select(ContactsAppState.statues).pipe(
    map(statuses => ({
      ...TOOLBAR_CONFIG,
      ...
      filterConfig: [
        ...
        },
        {
          ...
          label: 'contacts-app.actions.status.title',
          type: PeFilterType.Option,
          options:
            statuses?.map((status: StatusField): PeFilterKeyInterface['options'][number] => ({
              label: this.translateService.translate(status.name),
              value: status.id,
            })) ?? [],
        },
        ...TOOLBAR_CONFIG.filterConfig,
      ],
    })),
  );
  ...
  private readonly confirmationDialogOpener$ = this.messageBus.listen('confirm.dialog.open').pipe(
    tap((headings: Headings) => {
      this.confirmScreenService.show(headings, false);
    }),
  );

  public readonly gridItems$ = this.store.select(PeGridState.gridItems(APP_NAME)).pipe(
    skip(2),
    startWith([]),
    map(gridItems => {
      return gridItems.map(item => {
        const statuses: StatusField[] = this.store.selectSnapshot(ContactsAppState.statues);
        const status = statuses.find(status => item?.data?.status === status.id);
  ...
  private readonly initFoldersTree$ = this.peFoldersApiService.getFoldersTree().pipe(
    ...
    switchMap(() =>
      this.statusGQLService.getAllContactsStatus(this.envService.businessId).pipe(
        tap(statuses => {
  ...
  private readonly getFoldersTree$ = this.store.select(PeGridState.folders(APP_NAME)).pipe(
    tap(folders => {
  ...
  private readonly foldersChangeListener$ = this.peFoldersActionsService.folderChange$.pipe(
    tap(({ folder, action }) => {
  ...
  private readonly folderEditor$ = this.saveFolderSubject$.pipe(
    filter(data => {
      return Boolean(data);
    }),
    tap(({ actionType, updatedFolder }: PeFolderEditorDataToSaveInterface) => {
      ...
      if (actionType === PeFoldersActionsEnum.Create) {
        this.peFolderService.addFolder$.next(folder);
      }
      if (actionType === PeFoldersActionsEnum.Update) {
        this.peFolderService.updateFolder$.next(folder);
        this.onUpdateFolder(updatedFolder);
      }
  ...
  private readonly selectFolderListener$ = this.onSelectFolder$.pipe(
    skip(2),
  ...
  private readonly updateFolderListener$ = this.onUpdateFolder$.pipe(
    tap(folder => {
      this.viewportTitle = folder.name;
    }),
  );

  private readonly contactEditor$ = this.saveContactSubject$.pipe(
    skip(1),
    tap(contactData => {
      this.peOverlayRef.close();
      const contactItem: ContactMainInfo = getContactDisplayFields(contactData.contact);
      const contactId = contactData.contactId;
      const _id = contactId ? contactId.split('|')[0] : contactData.contact._id;
      const serviceEntityId = contactId ? contactId.split('|')[1] : contactItem.id;

      const gridItem = this.contactsListService.contactsToGridItemMapper([
        {
          ...contactItem,
          ...contactData.contact,
          _id,
  ...
  private readonly openEditor$ = this.editContactSubject$.pipe(
    skip(1),
    tap(contact => {
      this.peOverlayRef.close();
  ...
  private readonly gridSidenavToggle$ = this.messageBus.listen('contacts-app.grid-sidenav.toggle').pipe(
    tap(() => {
  ...
  private readonly deviceTypeChange$ = this.peGridViewportService.deviceTypeChange$.pipe(
  ...
  private readonly toggleOpenStatus$ = this.peGridSidenavService.toggleOpenStatus$.pipe(
    filter(() => !this.popupMode),
    tap((open: boolean) => {
      this.pePlatformHeaderService.assignConfig({
  ...
  constructor(
    ...
    this.pePreloaderService.initFinishObservers([this.isLoading$, this.isFoldersLoading$], APP_NAME);
    const view =
      this.peGridQueryParamsService.getQueryParamByName(GridQueryParams.View) ?? this.contactsListService.lastGridView;
    this.contactsListService.lastGridView = <PeGridView>view;
    this.gridLayout = <PeGridView>view;
  ...
  ngOnInit(): void {
    ...
    merge(
      this.contactsRulesService.initRuleListener(),
  ...
  private restructureFoldersTree(folder: FolderItem, action: PeFoldersActionsEnum): void {
    ...
    switch (action) {
      ...
      case PeFoldersActionsEnum.Update:
        if (!isExistingInGrid && isInSelectedFolder) {
          totalShift = 1;
        } else if (isExistingInGrid && !isInSelectedFolder) {
          totalShift = -1;
        } else {
          totalShift = 0;
        }
  ...
  public menuItemSelected(menuItem: PeGridMenuItem): void {
    switch (menuItem.value) {
      ...
      case SideNavMenuActions.Rules:
        this.contactsRulesService.openRules().pipe(
  ...
  public onUpdateFolder(folder: FolderItem): void {
    this.onUpdateFolder$.next(folder);
  }
    ...
    this.peFoldersApiService
      ...
      .pipe(
        tap(folderItems => {
          ...
          const folderChildren = isRootFolder
            ? this.rootTree
            : collection.filter((collectionItem: any) => collectionItem.isFolder);
          const prevGridItems =
          ...
          const items = collection.length
            ? this.contactsListService.contactsToGridItemMapper(
                collection.filter((collectionItem: any) => !collectionItem.isFolder),
              )
            : [];
  ...
  public folderAction(event: FolderOutputEvent, action: PeFoldersActionsEnum): void {
    if (action === PeFoldersActionsEnum.Update) {
      this.onUpdateFolder(event.data);
    }
  ...
  private showError(error: any): void {
    this.snackbarService.toggle(true, {
      content: this.translateService.translate(error?.message || 'errors.unknown_error'),
      duration: 5000,
  ...
  // check this hack: https://github.com/angular/components/issues/20194#issuecomment-671716432
  cdkDropListExited(e) {
    const isRootFolder = !this.selectedFolder?._id;
    if (!isRootFolder) {
      this.cdkDropListExitedCopyElementId$.next(null);

      return;
    }
    const itemId = e.item[0].id;
    const index = this.gridItems.findIndex(i => i.id === itemId);
    const item = this.gridItems[index];

    this.cdkDropListExitedCopyElementId$.next(`${item.id}-copy`);
    this.store.dispatch(
      new PeGridItemsActions.AddItemAt(
        {
          ...item,
          id: this.cdkDropListExitedCopyElementId$.value,
        },
        index + 1,
        APP_NAME,
      ),
  ...
  cdkDropListDropped() {
    if (!this.cdkDropListExitedCopyElementId$.value) {
      return;
    }
    this.store.dispatch(new PeGridItemsActions.DeleteItems([this.cdkDropListExitedCopyElementId$.value], APP_NAME));
    this.cdkDropListExitedCopyElementId$.next(null);
  }

  public moveToFolder(event: MoveIntoFolderEvent): void {
    const { folder, moveItems } = event;
    const sameFolder = folder._id === this.selectedFolder?._id;

    if (!moveItems?.length || sameFolder || !folder._id) {
      this.deselectAllItems();
          ...
          return config.data.customFields && config.data.customFields.length > 0
            ? forkJoin(
                config.data.customFields
                  ...
                  .map(field => this.fieldsService.deleteField(this.envService.businessId, field._id)),
              )
            : EMPTY;
        }),
        takeUntil(this.destroy$),
  ...
  public onItemContentContextMenu(event: PeGridItemContextSelect): void {
    ...
    switch (action) {
      ...
      case ContextMenu.Delete:
        ...
        break;
      case ContextMenu.moveToFolder:
        this.openMoveToFolderDialog(gridItem);
        break;
      case ContextMenu.AddFolder:
        this.createFolder();
        break;
  ...
  private openMoveToFolderDialog(gridItem: PeGridItem): void {
    const overlayData = {
      item: gridItem,
      theme: this.theme,
      onSelectFolder: this.onSelectFolder$,
    ...
    const config: PeOverlayConfig = {
      ...
      panelClass: 'contacts-widget-global-modal',
      data: overlayData,
    };
