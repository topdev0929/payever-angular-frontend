...
export class ContactsGQLService {
  ...
  public addContact(newContact: AddContact, folderId?: string): Observable<ContactResponse> {
    const businessId = this.envService.businessId || this.envService.businessData._id;
    const fieldsMutation: any = `${newContact.fields?.length
      ? `
        data: {
  ...
  public removeStoreItem(ids: string[]): Observable<Responses.Empty> {
    const businessId = this.envService.businessId || this.envService.businessData._id;
    const mutation = graphqlTag`
  ...
  public updateContactStatus(id: string, status: string): Observable<ContactResponse> {
    const businessId = this.envService.businessId || this.envService.businessData._id;
  ...
  getFlatFolders(): Observable<FolderItem[]> {
    return this.httpClient.get<FolderItem[]>(`${this.env.backend.contacts}/api/folders/business/${this.envService.businessId}`);
  }

  getFolders(): Observable<FolderItem[]> {
    return this.httpClient.get<FolderItem[]>(`${this.env.backend.contacts}/api/folders/business/${this.envService.businessId}/tree`);
  }
