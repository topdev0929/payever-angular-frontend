...
import { ContactStatusComponent } from '../contacts-status';
...
export class ContactComponent implements OnInit {
  ...
  public get statusFieldsWithIcon() {
    return this.statusFields.map(status => ({
      ...status,
      icon: {
   ...
   (window as any).PayeverStatic.IconLoader.loadIcons(['social']);
    ...
    this.onSavedSubject$.pipe(
      tap((data) => {
        ...
        if (data?.delete) {
          this.deleteStatusByIds(data?.value);
        }
      }),
  ...
  public get statusValue() {
    return this.statusFields.find(status =>
       status.id === this.contactForm.get('status').value
      )?.name;
  }
  ...
  private updateStatus(value) {
    ...
    this.statusFields[index].color = value.color;
  ...
  private deleteStatusByIds(ids: string[]) {
    ids?.forEach((id) => {
      const index = this.statusFields.findIndex(status => status.id === id);
      index !== -1 && this.statusFields.splice(index, 1);
    });
    this.cdr.markForCheck();
  }
  ...
  private onSave = (): void => {
    ...
    controls.mobilePhone.setValidators([Validators.pattern('^\d*$')]);
