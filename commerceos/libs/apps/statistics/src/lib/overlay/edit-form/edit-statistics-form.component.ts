...
export class PeStatisticsEditFormComponent implements OnInit, OnDestroy {
    ...
    metricsDone.subscribe(() => {
      ...
      this.overlayData.data.widgetSettings.forEach((item, index) => {
        ...
        item.forEach((element) => {
          ...
          if (element.type === 'daysAgo') {
            formFieldData['daysAgo'] = element.value;
            formFieldData['timeFrame'] = 'days_ago';
      ...
      this.addWidgetForm.valueChanges.pipe(
        tap(this.formPreviewDataSource),
          ...
          if (onSave) {
            ...
            if (this.addWidgetFormValue?.fields) {
              ...
              if (this.widgetService.viewType === 'LineGraph' && data.widgetSettings[1][1].length === 0) {
                ...
                this.cdr.detectChanges();
              } else if (this.widgetService.currentDashboard?.id) {
  ...
  gatherDataFromField(field, currentForm) {
    ...
    if (currentForm.daysAgo) {
      field.push({
        type: 'daysAgo',
        value: currentForm.daysAgo,
      });
    }
    ...
    Object.keys(currentForm).forEach((settingField) => {
      if (dimensions.includes(settingField)) {
        ...
        } else if (currentForm[settingField] !== 'all') {
          field.push({
            type: 'filter',
            value: {
              name: this.widgetService.dimensionTypes[settingField].name,
  ...
  onRemove() {
    this.showConfirmationDialog(
      this.translateService.translate('statistics.confirm_dialog.subtitle_delete_widget'),
    ).pipe(
      tap(() => {
        this.apiService
