...
export class LoginAsUserLayoutComponent implements OnInit {
  ...
  ngOnInit() {
    this.isByEmail = this.activatedRoute.snapshot.queryParams['isByEmail'] === 'true';
    this.forceRedirectToPersonal = this.activatedRoute.snapshot.queryParams['forceRedirectToPersonal'] === 'true';
    ...
    this.authService.setToken(this.activatedRoute.snapshot.queryParams['accessToken']).pipe(
      ...
      switchMap(() => {
        if (this.isByEmail) {
          ...
          if (this.forceRedirectToPersonal) {
