...
export class ForgotPasswordComponent {
    ...
    this.apiService.requestPasswordResetEmail(payload).subscribe(
      ...
      (exception: any) => {
        ...
        if (exception.error && ['REASON_DISPLAY_CAPTCHA', 'REASON_NO_CAPTCHA'].indexOf(exception.error.reason) >= 0) {
          ...
          this.errorMessage = this.translateService.hasTranslation(exception.error.message)
            ? this.translateService.translate(exception.error.message)
            : this.translateService.translate('forms.error.unknown_error');
        } else {
          this.errors.email.hasError = true;
          this.errors.email.errorMessage = this.translateService.translate('forms.error.unknown_error');
        }
        this.isLoading = false;
        this.cdr.markForCheck();
        this.cdr.detectChanges();
  ...
  checkErrors() {
    for (let control in this.form.controls) {
      if (this.form.controls[control].invalid && this.errors[control]) {
        ...
        if (this.form.controls[control].errors.required) {
          this.errors[control].errorMessage = this.translateService.translate(`forms.error.validator.${control}.required`);
        }
    ...
    this.checkRecaptchaTokenErrors();
  }

  checkRecaptchaTokenErrors() {
    const recaptchaTokenControl = this.form.get('recaptchaToken');
