...
import { CreateBusinessFormInterface } from '@pe/entry/business-form';
...
import { PeUser, StateUserService, UserLoaded, UserState } from '@pe/user';
...
export abstract class BaseBusinessFormComponent implements OnInit {
  ...
  private onboardingService: OnboardingService = this.injector.get(OnboardingService);
  private pluginOnboardingService: PluginOnboardingService = this.injector.get(PluginOnboardingService);
  ...
  private addCompany(businessId: string) {
    ...
    this.apiService.createCompany(createCompanyData).subscribe(
      (responseBusinessData: any) => {
        ...
        if (this.plugin) {
          this.afterRegistrationActions(RegisterStep.Business, responseBusinessData._id, '').pipe(
        ...
        this.apiService.enableBusiness(responseBusinessData._id).pipe(
          switchMap(tokens =>
            this.authService.setToken(tokens.accessToken).pipe(
              switchMap(() => {
                combineLatest([
                  this.onboardingService.actionController,
                ]).pipe(
                  tap(() => {
                    ...
                    if (invitationRedirectUrl) {
                      ...
                      this.stateUserService.isNewUser$.next(true);
                      this.router
  ...
  private afterRegistrationActions(step, id, re): Observable<unknown> {
    if (this.afterRegistrationMethods[step]) {
      return this.plugin
        ? this.pluginOnboardingService.runAfterPluginActions(this.afterRegistrationMethods[step], {
          businessId: id,
          integration: this.plugin,
        })
        : this.onboardingService.runAfterActions(this.afterRegistrationMethods[step], id, this.partnerData.name);
    }
