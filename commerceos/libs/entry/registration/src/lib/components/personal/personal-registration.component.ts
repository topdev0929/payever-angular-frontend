...
export class PersonalRegistrationComponent implements OnInit {
  ...
  @Output() navigateAfterSocial = new EventEmitter<CreatePersonalFormEventType>();
  ...
  ngOnInit() {
    this.industryIcon = this.entryLogo || this.registrationService.loadIndustryIcon(this.route.snapshot?.params.industry, entryLogo);
  }
  ...
  onFormEvent(e: CreatePersonalFormEvent): void {
    switch (e.event) {
      ...
      case CreatePersonalFormEventType.NavigateToLogin:
        this.goLogin.emit();
        break;
      case CreatePersonalFormEventType.NavigateAfterSocial:
        this.navigateAfterSocial.emit(e.data.type);
        break;
    }
  }
}
