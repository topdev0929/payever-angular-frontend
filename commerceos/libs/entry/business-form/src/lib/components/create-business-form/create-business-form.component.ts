...
import { OnboardingRequestDTO } from '@pe/shared/onboarding';
...
export class CreateBusinessFormComponent extends BaseBusinessFormComponent implements OnInit, AfterViewInit {
  ...
  get defaultCountryPhoneCode(): string {
    return this.countrySlug
      ? `${this.countrySlug}:+${countryTelephoneCodes[this.countrySlug]}`
      : this.DEFUALT_VALUE.countryPhoneCode;
  }

  ngOnInit(): void {
    this.initialIndustrySlug = (this.route.snapshot.params.industry || '').toLowerCase();
    this.countrySlug = OnboardingRequestDTO.mapCountry(this.route.snapshot.params.country)?.toUpperCase();
  ...
  createForm(): void {
    this.form = this.formBuilder.group({
      ...
      countryPhoneCode: [this.defaultCountryPhoneCode, Validators.required],
  ...
  private setCountryPhoneCode(): void {
    ...
    this.form.get('countryPhoneCode').setValue(this.countrySlug
      ? this.defaultCountryPhoneCode
      : defaultCountryPhoneCode || this.DEFUALT_VALUE.countryPhoneCode
    );
  }
