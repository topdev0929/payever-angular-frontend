...
import { MoveIntoFolderEvent } from '@pe/folders';
...
export class PeGridContentComponent {
  ...
  @HostBinding('class.pe-grid-content_opened') get isOpenSidenav() {
    if (this.sidenavName === DEFAULT_SIDENAV) {
      return this.peGridSidenavService.toggleOpenStatus$.value
    }
    if (this.peGridSidenavService.sidenavOpenStatus?.[this.sidenavName]) {
      return this.peGridSidenavService.sidenavOpenStatus[this.sidenavName].value
    }
