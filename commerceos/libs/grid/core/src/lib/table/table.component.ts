...
import { distinctUntilChanged, filter, map, skip, switchMap, takeUntil, tap } from 'rxjs/operators';

import { AppType, APP_TYPE, PeDestroyService, PeHelpfulService, PreloaderState } from '@pe/common';

import { FadeOutAnimation } from '../grid.animations';
...
export class PeGridTableComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('infiniteScrollLoading', { static: false }) infiniteScrollLoading: ElementRef;

  virtualScrollTableWrapperRef: ElementRef;
  @ViewChild('virtualScrollTableWrapper', { static: false }) set setVirtualScrollTableWrapper(element: ElementRef) {
    this.virtualScrollTableWrapperRef = element;
  ...
  onScrolledToEnd(){
    ...
    window.requestAnimationFrame(() => {
      this.infiniteScrollLoading.nativeElement.scrollIntoView({ behavior:'smooth' });
    });
  }
      ...
      this.peGridService.items$.pipe(
        ...
        tap((items: PeGridItem[]) => {
          ...
          this.items = items.map((item) => {
            if (item?.image && !item?.useDefaultImage) {
  ...
  initWrapperSize() {
    ...
    if (this.virtualScrollTableWrapperRef) {
      this.resizeObserverWrapper.observe(this.virtualScrollTableWrapperRef.nativeElement);
    }
  }

  get isShowSkeleton(): boolean {
