...
export class PeGridMenuService {
  ...
  open(
    ...
    this.overlayRef = this.overlay.create({
      ...
      panelClass: 'pe-grid-menu__overlay',
  ...
  openSearch(menu: PeGridMenu): OverlayRef {
    ...
    const overlayRef: OverlayRef = this.overlay.create({
      hasBackdrop: true,
      positionStrategy,
      backdropClass: ['pe-grid-menu__search-backdrop','pe-overlay-widget-backdrop'],
    });
    const menuPortal = new ComponentPortal(PeGridMenuComponent);
    ...
    menuRef.instance.mobileFullscreen = true;
  ...
  private getPositions(position: PeGridMenuPosition): ConnectedPosition {
    switch (position) {
      ...
      case PeGridMenuPosition.CenterBottom:
