...
export class PeGridState {
  ...
  private flatFoldersTree(folders: FolderItem[]): FolderItem[] {
    return folders.some(folder => folder?.children?.length)
  ...
  @Action(PeGridItemsActions.AddItemAt)
  addItemAt(
    { patchState, getState }: StateContext<PeGridStateInterface>,
    { newItem, index, appName }: PeGridItemsActions.AddItemAt
  ) {
    const state = produce(getState(), (draft: PeGridStateInterface) => {
      draft[appName].gridItems = [
        ...draft[appName].gridItems.slice(0, index),
        newItem,
        ...draft[appName].gridItems.slice(index),
      ];
    });
    patchState(state);
  }
  ...
  openFolder(
    { patchState, getState }: StateContext<PeGridStateInterface>,
    { folderItems, appName, isLiveChat }: PeGridItemsActions.OpenFolder
  ) {
    const state = produce(getState(), (draft: PeGridStateInterface) => {
      draft[appName].gridItems = isLiveChat ? [ ...draft[appName].gridItems ] : folderItems;
    });
    patchState(state);
  }
}
