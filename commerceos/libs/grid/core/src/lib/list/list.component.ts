...
export class PeGridListComponent implements AfterViewInit, AfterViewChecked, OnDestroy {
  ...
  private readonly imageHeight$ = new BehaviorSubject(
    isPlatformBrowser(this.platformId) ? window.innerWidth <= 720 ? 148 : 200 : 160);

  private readonly rowHeight$ = new BehaviorSubject(
    isPlatformBrowser(this.platformId) ? window.innerWidth <= 720 ? 148 : 200 : 200);
  ...
  @HostBinding('style') get columnsCount() {
    ...
    const gapSize = this.getGapSize(this.columns);
  ...
  private getGapSize(columns: number): string {
    const gapSizes = {
      2: '16px',
      3: '12px',
      4: '10px',
