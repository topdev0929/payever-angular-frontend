...
export class PeGridToolbarComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  @Input() isColumnsItems = true;
  @Input() isSortItems = true;
  @Input() isFilterIcon = true;
  @Input() isGridContentCount = true;
  @Input() columnSwitcherClass = '';
  ...
  openColumnSwitcher(element: any): void {
    ...
    this.peGridMenuService.open(element, columnsMenu, {
      ...config,
      classList: this.columnSwitcherClass,
    });
  ...
  private registerResizeToolbar(): void {
    this.resizeObserver = new ResizeObserver((entries) => {
      window.requestAnimationFrame(() => {
        ...
        this.isMobileView = (entries[0].target as HTMLDivElement).offsetWidth <= 461;
