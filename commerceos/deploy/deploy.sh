...
sed_pattern+=$(printenv | grep -e '^OLD_COMMERCEOS_TAG' | sed -e 's/\.//g' | awk '{sub(/=/," ");$1=$1;print " -e s#"$1"#"$2"#g"}')
...
if [[ $DEPLOY_TO_EXCLUSIVE != *"true"* ]] ; then
  ...
  echo "commerceos_verion is: $commerceos_version"
  ...
  if [[ ! -z $OLD_COMMERCEOS_TAG ]] ; then
    ./azcopy copy "$MICRO_URL_CUSTOM_STORAGE/cdn/commerceos/$OLD_COMMERCEOS_TAG$STORAGE_BLOB_SAS_KEY" "$MICRO_URL_CUSTOM_STORAGE/cdn/commerceos/$commerceos_version/oldcommerceos$STORAGE_BLOB_SAS_KEY" --recursive=true
  fi
...
if [[ $DEPLOY_TO_EXCLUSIVE == *"true"* ]] ; then
  ...
  if [ -n "$CUSTOM_CHECKOUT_CDN_URL" ]; then 
    echo "Replacing /payever/env.* with $CUSTOM_CHECKOUT_CDN_URL"; 
    echo $(grep -rl "https://checkout.payever.org" /payever | grep -v "deploy")
    sed -i "s|https://checkout.payever.org|$CUSTOM_CHECKOUT_CDN_URL|g" $(grep -rl "https://checkout.payever.org" /payever | grep -v "deploy"); 
    else 
    echo " CUSTOM_CHECKOUT_CDN_URL is empty "; 
  fi
  echo -e "\nStarting nginx\n"
  nginx -g 'daemon off;'
fi
