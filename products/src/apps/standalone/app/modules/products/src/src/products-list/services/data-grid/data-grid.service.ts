...
import { distinctUntilChanged, map, skip, startWith, switchMap, take, tap, withLatestFrom } from 'rxjs/operators';
...
import { Components } from '../../../shared/enums/app.enum';
...
export class DataGridService extends AbstractService implements OnDestroy {
  ...
  addActions: PeDataGridSingleSelectedAction[] = [
    {
      ...
      callback: () => {
        ...
        this.router.navigate(
          [...['business', this.envService.businessUuid, 'products', 'list'], { outlets: { editor: ['products-editor', 'add'] } }],
    ...
    menuItems: [
      {
        ...
        onClick: () => {
          this.router.navigate(
            [...['business', this.envService.businessUuid, 'products', 'list'], { outlets: { editor: 'collections-editor' } }],
            this.getNavigateParams(),
          );
        },
      },
    ...
    return this.productsListService.collections$.pipe(
      map(collections => {
        ...
        const treeData = collections
          ?.filter(collection => {
            /** temporary remove dropshiopping */
            // if (collection.slug === 'payever-dropshipping' || collection.slug === 'My_Dropshipping') {
            //   collection.image = 'assets/icons/market.svg';
            //   channelTreeData.push(collection);
            // }
  ...
  collectionEdit(id: string, isLoading?: BehaviorSubject<boolean>) {
    ...
    this.router.navigate([
      ...['business', this.envService.businessUuid, 'products', 'list'],
      { outlets: { editor: ['collections-editor', id] } },
    ], this.getNavigateParams()).then(() => isLoading?.next(false));
  }

  pasteItems(parentId = null, prefix?: string) {
  ...
  updateGrid(response?) {
    if (response?._id) {
      this.selectedFolder = response._id;
    }
    this.productsListService.loadCollections().pipe(
      take(1),
      tap(() => {
        const tree = this.collections.find(collection => collection.id === this.selectedFolder);
        const value = tree ? [tree] : [];
        this.expandTree = { expand: true, item: response === 'product' ? tree : response };
        this.filtersFormGroup.get('tree').patchValue(value);
        this.updateGridItems();
      }),
    ).subscribe();
  }
  ...
  updateGridFolders(parentId = null) {
    this.productsApiService.loadCollections(1, this.envService.businessUuid, parentId).pipe(
      take(1),
      tap(data => {
  ...
  updateGridItems() {
    const filters = this.selectedFolder ? [this.getCollectionsFilters(this.filtersFormGroup.get('tree').value)] : [];
    this.productsListService.loadProducts(filters, false, this.layout).pipe(take(1)).subscribe();
  }
  ...
  deleteSelected(collections: string[], products: string[], view: PeDataGridLayoutType) {
    ...
    this.confirmDialog.confirmation$.pipe(
      ...
      switchMap(() => this.productsApiService.deleteCollections(collections, this.envService.businessUuid)),
      ...
      switchMap(() => {
        return this.productsListService.loadProductsByChannelSet(this.envService.businessUuid);
      }),
      switchMap(() => {
        this.filtersFormGroup.get('tree').patchValue([]);
  ...
  addProduct(product?) {
    ...
    this.router.navigate([...['business', this.envService.businessUuid, 'products', 'list'],
