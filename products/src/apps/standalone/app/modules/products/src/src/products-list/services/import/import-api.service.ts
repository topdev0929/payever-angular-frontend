...
export class ImportApiService {
  businessId: string = this.envService.businessUuid;
  ...
  importFromMarket(businessId: string, integrationId: string, overwrite: boolean): Observable<any> {
    const apiURL = `${this.env.backend.synchronizer}/api/synchronization/business/${businessId}/integration/${integrationId}/direction/inward/trigger`;
  ...
  importFromFile(
    ...
    const apiURL = `${this.env.backend.synchronizer}/api/synchronization/business/${businessId}/tasks`;
  ...
  getTask(businessId: string, taskId: string): Observable<any> {
    const apiURL = `${this.env.backend.synchronizer}/api/synchronization/business/${businessId}/tasks/${taskId}`;
    return this.http.get<any>(apiURL);
  }
